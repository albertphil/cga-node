...
inline cga::build_flags operator| (cga::build_flags a, cga::build_flags b)
{
	return static_cast<cga::build_flags> (static_cast<uint8_t> (a) | static_cast<uint8_t> (b));
}
inline uint8_t operator| (uint8_t a, cga::build_flags b)
{
...
public:
	...
	inline abstract_builder & sign (cga::raw_key const & private_key, cga::public_key const & public_key)
	{
		block->signature = cga::sign_message (private_key, public_key, block->hash ());
...
protected:
	...
	uint8_t base_fields = static_cast<uint8_t> (cga::build_flags::work_present | cga::build_flags::signature_present);
};

/** Builder for state blocks */
class state_block_builder : public abstract_builder<cga::state_block, state_block_builder>
{
public:
	...
	/** Set link */
	state_block_builder & link (cga::uint256_union link);
	/** Set link from hex representation */
	state_block_builder & link_hex (std::string link_hex);
	/** Set link from an cga_ address */
	state_block_builder & link_address (std::string link_address);
	/** Provides validation for build() */
	void validate ();

private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (cga::build_flags::account_present | cga::build_flags::balance_present | cga::build_flags::link_present | cga::build_flags::previous_present | cga::build_flags::representative_present);
};

/** Builder for open blocks */
class open_block_builder : public abstract_builder<cga::open_block, open_block_builder>
{
public:
	...
	/** Set account */
	open_block_builder & account (cga::account account);
	/** Set account from hex representation of public key */
	open_block_builder & account_hex (std::string account_hex);
	/** Set account from an or cga_ address */
	open_block_builder & account_address (std::string account_address);
	/** Set representative */
	open_block_builder & representative (cga::account account);
	/** Set representative from hex representation of public key */
	...
	open_block_builder & source (cga::block_hash source);
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (cga::build_flags::account_present | cga::build_flags::representative_present | cga::build_flags::link_present);
};

/** Builder for change blocks */
class change_block_builder : public abstract_builder<cga::change_block, change_block_builder>
{
public:
	...
	/** Set representative */
	change_block_builder & representative (cga::account account);
	/** Set representative from hex representation of public key */
	...
	/** Set previous block hash */
	change_block_builder & previous (cga::block_hash previous);
	/** Set previous block hash from hex representation */
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (cga::build_flags::previous_present | cga::build_flags::representative_present);
};

/** Builder for send blocks */
class send_block_builder : public abstract_builder<cga::send_block, send_block_builder>
{
public:
...
class receive_block_builder : public abstract_builder<cga::receive_block, receive_block_builder>
{
public:
	...
	/** Set previous block hash */
	receive_block_builder & previous (cga::block_hash previous);
	/** Set previous block hash from hex representation */
	receive_block_builder & previous_hex (std::string previous_hex);
	/** Set source block hash */
	receive_block_builder & source (cga::block_hash source);
	/** Set source block hash from hex representation */
...
public:
	...
	inline cga::state_block_builder & state ()
	{
	...
	inline cga::open_block_builder & open ()
	{
	...
	inline cga::change_block_builder & change ()
	{
	...
	inline cga::send_block_builder & send ()
	{
	...
	inline cga::receive_block_builder & receive ()
	{
...
private:
	cga::state_block_builder state_builder;
	cga::open_block_builder open_builder;
	cga::change_block_builder change_builder;
	cga::send_block_builder send_builder;
	cga::receive_block_builder receive_builder;
};
}
