...
std::mutex cga::random_pool::mutex;
CryptoPP::AutoSeededRandomPool cga::random_pool::pool;

void cga::random_pool::generate_block (unsigned char * output, size_t size)
{
	std::lock_guard<std::mutex> lk (mutex);
	pool.GenerateBlock (output, size);
}

unsigned cga::random_pool::generate_word32 (unsigned min, unsigned max)
{
	std::lock_guard<std::mutex> lk (mutex);
	return pool.GenerateWord32 (min, max);
}

unsigned char cga::random_pool::generate_byte ()
{
	std::lock_guard<std::mutex> lk (mutex);
...
void cga::uint256_union::encode_account (std::string & destination_a) const
{
	...
	cga::uint512_t number_l (number ());
	number_l <<= 40;
	number_l |= cga::uint512_t (check);
	for (auto i (0); i < 60; ++i)
	{
...
std::string cga::uint256_union::to_account () const
{
	std::string result;
...
bool cga::uint256_union::decode_account (std::string const & source_a)
{
		...
		if (!error)
		{
			if (cga_prefix)
			{
				...
				if (*i == '1' || *i == '3')
				{
					...
					if (!error)
					{
						*this = (number_l >> 40).convert_to<cga::uint256_t> ();
...
cga::uint256_union::uint256_union (cga::uint256_t const & number_a)
{
	bytes.fill (0);
	boost::multiprecision::export_bits (number_a, bytes.rbegin (), 8, false);
}

bool cga::uint256_union::operator== (cga::uint256_union const & other_a) const
{
	return bytes == other_a.bytes;
}

// Construct a uint256_union = AES_ENC_CTR (cleartext, key, iv)
void cga::uint256_union::encrypt (cga::raw_key const & cleartext, cga::raw_key const & key, uint128_union const & iv)
{
...
bool cga::uint256_union::is_zero () const
{
	return qwords[0] == 0 && qwords[1] == 0 && qwords[2] == 0 && qwords[3] == 0;
}

std::string cga::uint256_union::to_string () const
{
	std::string result;
	encode_hex (result);
	return result;
...
cga::uint256_union & cga::uint256_union::operator^= (cga::uint256_union const & other_a)
{
...
cga::uint256_union cga::uint256_union::operator^ (cga::uint256_union const & other_a) const
{
...
cga::uint256_union::uint256_union (std::string const & hex_a)
{
...
void cga::uint256_union::clear ()
{
	qwords.fill (0);
}

cga::uint256_t cga::uint256_union::number () const
{
	cga::uint256_t result;
	boost::multiprecision::import_bits (result, bytes.begin (), bytes.end (), false);
	return result;
}

void cga::uint256_union::encode_hex (std::string & text) const
{
	assert (text.empty ());
...
bool cga::uint256_union::decode_hex (std::string const & text)
{
	auto error (false);
...
void cga::uint256_union::encode_dec (std::string & text) const
{
	assert (text.empty ());
	std::stringstream stream;
...
bool cga::uint256_union::decode_dec (std::string const & text)
{
...
cga::uint256_union::uint256_union (uint64_t value0)
{
	*this = cga::uint256_t (value0);
}
...
bool cga::uint512_union::operator== (cga::uint512_union const & other_a) const
{
	return bytes == other_a.bytes;
}

cga::uint512_union::uint512_union (cga::uint256_union const & upper, cga::uint256_union const & lower)
{
	uint256s[0] = upper;
	uint256s[1] = lower;
}

cga::uint512_union::uint512_union (cga::uint512_t const & number_a)
{
	bytes.fill (0);
	boost::multiprecision::export_bits (number_a, bytes.rbegin (), 8, false);
}

bool cga::uint512_union::is_zero () const
{
	return qwords[0] == 0 && qwords[1] == 0 && qwords[2] == 0 && qwords[3] == 0
	&& qwords[4] == 0 && qwords[5] == 0 && qwords[6] == 0 && qwords[7] == 0;
}

void cga::uint512_union::clear ()
{
	bytes.fill (0);
}

cga::uint512_t cga::uint512_union::number () const
{
	cga::uint512_t result;
	boost::multiprecision::import_bits (result, bytes.begin (), bytes.end (), false);
	return result;
}

void cga::uint512_union::encode_hex (std::string & text) const
{
	assert (text.empty ());
	std::stringstream stream;
...
bool cga::uint512_union::decode_hex (std::string const & text)
{
...
cga::uint512_union & cga::uint512_union::operator^= (cga::uint512_union const & other_a)
{
...
std::string cga::uint512_union::to_string () const
{
	std::string result;
	encode_hex (result);
	return result;
...
bool cga::raw_key::operator== (cga::raw_key const & other_a) const
{
...
void cga::raw_key::decrypt (cga::uint256_union const & ciphertext, cga::raw_key const & key_a, uint128_union const & iv)
{
...
cga::uint512_union cga::sign_message (cga::raw_key const & private_key, cga::public_key const & public_key, cga::uint256_union const & message)
{
	cga::uint512_union result;
	ed25519_sign (message.bytes.data (), sizeof (message.bytes), private_key.data.bytes.data (), public_key.bytes.data (), result.bytes.data ());
	return result;
}

void cga::deterministic_key (cga::uint256_union const & seed_a, uint32_t index_a, cga::uint256_union & prv_a)
{
	blake2b_state hash;
	blake2b_init (&hash, prv_a.bytes.size ());
	blake2b_update (&hash, seed_a.bytes.data (), seed_a.bytes.size ());
	cga::uint256_union index (index_a);
...
cga::public_key cga::pub_key (cga::private_key const & privatekey_a)
{
	cga::uint256_union result;
...
bool cga::validate_message (cga::public_key const & public_key, cga::uint256_union const & message, cga::uint512_union const & signature)
{
	auto result (0 != ed25519_sign_open (message.bytes.data (), sizeof (message.bytes), public_key.bytes.data (), signature.bytes.data ()));
	return result;
}

bool cga::validate_message_batch (const unsigned char ** m, size_t * mlen, const unsigned char ** pk, const unsigned char ** RS, size_t num, int * valid)
{
	bool result (0 == ed25519_sign_open_batch (m, mlen, pk, RS, num, valid));
	return result;
}

cga::uint128_union::uint128_union (std::string const & string_a)
{
...
cga::uint128_union::uint128_union (uint64_t value_a)
{
	*this = cga::uint128_t (value_a);
}

cga::uint128_union::uint128_union (cga::uint128_t const & number_a)
{
	bytes.fill (0);
	boost::multiprecision::export_bits (number_a, bytes.rbegin (), 8, false);
}

bool cga::uint128_union::operator== (cga::uint128_union const & other_a) const
{
...
cga::uint128_t cga::uint128_union::number () const
{
...
void cga::uint128_union::encode_hex (std::string & text) const
{
	assert (text.empty ());
	std::stringstream stream;
...
bool cga::uint128_union::decode_hex (std::string const & text)
{
...
void cga::uint128_union::encode_dec (std::string & text) const
{
	assert (text.empty ());
	std::stringstream stream;
	stream << std::dec << std::noshowbase;
...
bool cga::uint128_union::decode_dec (std::string const & text)
{
	...
	if (!error)
	{
		...
		{
			stream >> number_l;
			cga::uint128_t unchecked (number_l);
			*this = unchecked;
			if (!stream.eof ())
			{
				error = true;
...
void format_frac (std::ostringstream & stream, cga::uint128_t value, cga::uint128_t scale, int precision)
{
	auto reduce = scale;
	auto rem = value;
...
void format_dec (std::ostringstream & stream, cga::uint128_t value, char group_sep, const std::string & groupings)
{
	auto largestPow10 = cga::uint256_t (1);
	int dec_count = 1;
	while (1)
	{
	...
	auto reduce = cga::uint128_t (largestPow10);
	cga::uint128_t rem = value;
	while (reduce > 0)
	{
		auto val = rem / reduce;
		rem -= val * reduce;
...
std::string format_balance (cga::uint128_t balance, cga::uint128_t scale, int precision, bool group_digits, char thousands_sep, char decimal_point, std::string & grouping)
{
...
std::string cga::uint128_union::format_balance (cga::uint128_t scale, int precision, bool group_digits)
{
...
std::string cga::uint128_union::format_balance (cga::uint128_t scale, int precision, bool group_digits, const std::locale & locale)
{
...
void cga::uint128_union::clear ()
{
	qwords.fill (0);
}

bool cga::uint128_union::is_zero () const
{
	return qwords[0] == 0 && qwords[1] == 0;
}

std::string cga::uint128_union::to_string () const
{
	std::string result;
	encode_hex (result);
	return result;
...
std::string cga::uint128_union::to_string_dec () const
{
	std::string result;
	encode_dec (result);
	return result;
