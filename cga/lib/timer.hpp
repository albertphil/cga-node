...
public:
	...
	timer (cga::timer_state state_a, std::string description_a = "timer") :
	desc (description_a)
	{
		if (state_a == cga::timer_state::started)
		{
			start ();
		}
	}
	...
	void start ()
	{
		assert (state == cga::timer_state::stopped);
		state = cga::timer_state::started;
		begin = CLOCK::now ();
	}
	...
	UNIT stop ()
	{
		assert (state == cga::timer_state::started);
		state = cga::timer_state::stopped;
	...
	cga::timer_state current_state () const
	{
		return state;
	}
...
private:
	...
	cga::timer_state state{ cga::timer_state::stopped };
	std::string desc;
...
class autotimer : public cga::timer<UNIT>
{
public:
	autotimer (std::string description_a, std::ostream & stream_a = std::cout) :
	cga::timer<UNIT> (description_a), stream (stream_a)
	{
		cga::timer<UNIT>::start ();
	}
	~autotimer ()
	{
		cga::timer<UNIT>::stop (stream);
	}
