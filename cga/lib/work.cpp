...
bool cga::work_validate (cga::block_hash const & root_a, uint64_t work_a, uint64_t * difficulty_a)
{
	auto value (cga::work_value (root_a, work_a));
	if (difficulty_a != nullptr)
	{
		*difficulty_a = value;
	}
	return value < cga::work_pool::publish_threshold;
}

bool cga::work_validate (cga::block const & block_a, uint64_t * difficulty_a)
{
	return work_validate (block_a.root (), block_a.block_work (), difficulty_a);
}

uint64_t cga::work_value (cga::block_hash const & root_a, uint64_t work_a)
{
	uint64_t result;
	blake2b_state hash;
...
cga::work_pool::work_pool (unsigned max_threads_a, std::function<boost::optional<uint64_t> (cga::uint256_union const &)> opencl_a) :
ticket (0),
done (false),
opencl (opencl_a)
{
	...
	auto count (cga::is_test_network ? 1 : std::min (max_threads_a, std::max (1u, boost::thread::hardware_concurrency ())));
	for (auto i (0); i < count; ++i)
	{
		auto thread (boost::thread (attrs, [this, i]() {
			cga::thread_role::set (cga::thread_role::name::work);
			cga::work_thread_reprioritize ();
			loop (i);
		}));
...
void cga::work_pool::loop (uint64_t thread)
{
	// Quick RNG for work attempts.
	xorshift1024star rng;
	cga::random_pool::generate_block (reinterpret_cast<uint8_t *> (rng.s.data ()), rng.s.size () * sizeof (decltype (rng.s)::value_type));
	uint64_t work;
	uint64_t output;
	blake2b_state hash;
	...
	while (!done || !pending.empty ())
	{
		...
		if (!empty)
		{
			...
			if (ticket == ticket_l)
			{
				...
				assert (output >= cga::work_pool::publish_threshold);
...
void cga::work_pool::cancel (cga::uint256_union const & root_a)
{
...
void cga::work_pool::generate (cga::uint256_union const & root_a, std::function<void(boost::optional<uint64_t> const &)> callback_a, uint64_t difficulty_a)
{
...
uint64_t cga::work_pool::generate (cga::uint256_union const & hash_a, uint64_t difficulty_a)
{
