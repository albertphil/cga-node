...
REGISTER_ERROR_CODES (cga, error_common);
REGISTER_ERROR_CODES (cga, error_blocks);
REGISTER_ERROR_CODES (cga, error_rpc);
REGISTER_ERROR_CODES (cga, error_process);
REGISTER_ERROR_CODES (cga, error_config);
...
inline std::error_code make_error_code (boost::system::errc::errc_t e)
{
	...
	::cga::error_conversion::generic_category ());
}
}
namespace cga
{
namespace error_conversion
{
	...
	inline std::error_code convert (const boost::system::error_code & error)
	{
		...
		return cga::error_common::invalid_type_conversion;
	}
}
}
...
public:
	error () = default;
	error (cga::error const & error_a) = default;
	error (cga::error && error_a) = default;
	...
	{
		code = cga::error_common::generic;
		message = std::move (message_a);
	}

	error (std::exception const & exception_a)
	{
		code = cga::error_common::exception;
		message = exception_a.what ();
	}

	error & operator= (cga::error const & err_a)
	{
		code = err_a.code;
	...
	error & operator= (cga::error && err_a)
	{
		code = err_a.code;
	...
	{
		code = cga::error_conversion::convert (code_a);
		message.clear ();
		return *this;
	}
	...
	{
		code = cga::error_conversion::convert (boost::system::errc::make_error_code (code_a));
		message.clear ();
		return *this;
	}

	/** Set the error to cga::error_common::generic and the error message to \p message_a */
	...
	/** Set the error to cga::error_common::generic. */
	...
	/** Sets the error to cga::error_common::exception and adopts the exception error message. */
	...
	error & then (std::function<cga::error &()> next)
	{
	...
	error & set (std::string message_a, std::error_code code_a = cga::error_common::generic)
	{
		message = message_a;
		code = code_a;
		return *this;
	...
	/** Set a custom error message. If the error code is not set, it will be set to cga::error_common::generic. */
...
/**
 * A type that manages a cga::error.
 * The default return type is cga::error&, though shared_ptr<cga::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = cga::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, cga::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<cga::error>>::value, "Must be cga::error& or shared_ptr<cga::error>");
