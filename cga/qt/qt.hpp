...
public:
	...
	QPushButton * back;
	cga_qt::wallet & wallet;
};
class advanced_actions
{
public:
	advanced_actions (cga_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
	...
	QRadioButton * mcga_unit;
	QRadioButton * kcga_unit;
	QRadioButton * cga_unit;
	QPushButton * back;
...
{
public:
	self_pane (cga_qt::wallet &, cga::account const &);
	void set_balance_text (std::pair<cga::uint128_t, cga::uint128_t>);
	QWidget * window;
	QVBoxLayout * layout;
...
{
public:
	history (cga::ledger &, cga::account const &, cga_qt::wallet &);
	void refresh ();
	QWidget * window;
	QVBoxLayout * layout;
	QStandardItemModel * model;
	...
	QSpinBox * tx_count;
	cga::ledger & ledger;
	cga::account const & account;
	cga_qt::wallet & wallet;
};
class block_viewer
{
public:
	block_viewer (cga_qt::wallet &);
	void rebroadcast_action (cga::uint256_union const &);
	QWidget * window;
	QVBoxLayout * layout;
	QLabel * hash_label;
	QLineEdit * hash;
...
{
public:
	status (cga_qt::wallet &);
	void erase (cga_qt::status_types);
	void insert (cga_qt::status_types);
	void set_text ();
	std::string text ();
	std::string color ();
	std::set<cga_qt::status_types> active;
	cga_qt::wallet & wallet;
};
class wallet : public std::enable_shared_from_this<cga_qt::wallet>
{
public:
	wallet (QApplication &, cga_qt::eventloop_processor &, cga::node &, std::shared_ptr<cga::wallet>, cga::account &);
	void start ();
	void refresh ();
	void update_connected ();
	void empty_password ();
	void change_rendering_ratio (cga::uint128_t const &);
	std::string format_balance (cga::uint128_t const &) const;
	cga::uint128_t rendering_ratio;
	cga::node & node;
	std::shared_ptr<cga::wallet> wallet_m;
	cga::account & account;
	cga_qt::eventloop_processor & processor;
	cga_qt::history history;
	cga_qt::accounts accounts;
	cga_qt::self_pane self;
	cga_qt::settings settings;
	cga_qt::advanced_actions advanced;
	cga_qt::block_creation block_creation;
	cga_qt::block_entry block_entry;
	cga_qt::block_viewer block_viewer;
	cga_qt::account_viewer account_viewer;
	cga_qt::stats_viewer stats_viewer;
	cga_qt::import import;
