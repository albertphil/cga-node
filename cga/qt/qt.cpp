...
bool cga_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<cga_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<cga_qt::eventloop_event *> (event_a)->action ();
	return true;
}

cga_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

cga_qt::self_pane::self_pane (cga_qt::wallet & wallet_a, cga::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel ("Your CGA account:")),
...
wallet (wallet_a)
{
	...
	if (cga::is_beta_network)
	{
		network = "Beta";
	}
	else if (cga::is_test_network)
	{
		network = "Test";
	}
	if (CGA_VERSION_PATCH == 0)
	{
		version = new QLabel (boost::str (boost::format ("Version %1% %2% network") % CGA_MAJOR_MINOR_VERSION % network).c_str ());
	}
	else
	{
		version = new QLabel (boost::str (boost::format ("Version %1% %2% network") % CGA_MAJOR_MINOR_RC_VERSION % network).c_str ());
	}
...
void cga_qt::self_pane::set_balance_text (std::pair<cga::uint128_t, cga::uint128_t> balance_a)
{
...
cga_qt::accounts::accounts (cga_qt::wallet & wallet_a) :
...
void cga_qt::accounts::refresh_wallet_balance ()
{
	auto transaction (this->wallet.wallet_m->wallets.tx_begin_read ());
...
cga_qt::import::import (cga_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
cga_qt::history::history (cga::ledger & ledger_a, cga::account const & account_a, cga_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
namespace
{
class short_text_visitor : public cga::block_visitor
{
public:
	short_text_visitor (cga::transaction const & transaction_a, cga::ledger & ledger_a) :
	...
	void open_block (cga::open_block const & block_a)
	{
		...
		{
			account = cga::genesis_account;
			amount = cga::genesis_amount;
		}
	}
...
cga_qt::block_viewer::block_viewer (cga_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void cga_qt::block_viewer::rebroadcast_action (cga::uint256_union const & hash_a)
{
	auto done (true);
...
cga_qt::account_viewer::account_viewer (cga_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (CGA): ") + wallet.format_balance (balance.first));
...
cga_qt::stats_viewer::stats_viewer (cga_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
refresh (new QPushButton ("Refresh")),
model (new QStandardItemModel),
...
void cga_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
cga_qt::status::status (cga_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (cga_qt::status_types::nominal);
	set_text ();
}

void cga_qt::status::erase (cga_qt::status_types status_a)
{
	assert (status_a != cga_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void cga_qt::status::insert (cga_qt::status_types status_a)
{
	assert (status_a != cga_qt::status_types::nominal);
...
std::string cga_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case cga_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case cga_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case cga_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case cga_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case cga_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case cga_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case cga_qt::status_types::nominal:
...
std::string cga_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
cga_qt::wallet::wallet (QApplication & application_a, cga_qt::eventloop_processor & processor_a, cga::node & node_a, std::shared_ptr<cga::wallet> wallet_a, cga::account & account_a) :
rendering_ratio (cga::Mcga_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
needs_deterministic_restore (false)
{
	...
	send_account->setPlaceholderText (cga::zero_key.pub.to_account ().c_str ());
...
void cga_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<cga_qt::wallet> wallet_w (shared_from_this ());
...
void cga_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<cga_qt::wallet> this_w (shared_from_this ());
	...
	QObject::connect (send_blocks_send, &QPushButton::released, [this_w]() {
		if (auto this_l = this_w.lock ())
		{
			...
			{
				cga::uint128_t actual (amount.number () * this_l->rendering_ratio);
						...
						if (actual <= balance)
						{
							...
							if (this_l->wallet_m->store.valid_password (transaction))
							{
								...
								this_l->node.background ([this_w, account_l, actual]() {
									if (auto this_l = this_w.lock ())
									{
										this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<cga::block> block_a) {
											if (auto this_l = this_w.lock ())
											{
	...
	node.observers.blocks.add ([this_w](std::shared_ptr<cga::block> block_a, cga::account const & account_a, cga::uint128_t const & amount_a, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w](cga::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (cga_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (cga_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](cga::endpoint const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (cga_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (cga_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (cga_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (cga_qt::status_types::working);
					}
				}
			}));
		}
...
void cga_qt::wallet::update_connected ()
{
	if (node.peers.empty ())
	{
		active_status.insert (cga_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (cga_qt::status_types::disconnected);
	}
}

void cga_qt::wallet::empty_password ()
{
...
void cga_qt::wallet::change_rendering_ratio (cga::uint128_t const & rendering_ratio_a)
{
...
std::string cga_qt::wallet::format_balance (cga::uint128_t const & balance) const
{
	auto balance_str = cga::amount (balance).format_balance (rendering_ratio, 0, false);
	auto unit = std::string ("CGA");
	if (rendering_ratio == cga::kcga_ratio)
	{
		unit = std::string ("kcga");
	}
	else if (rendering_ratio == cga::cga_ratio)
	{
		unit = std::string ("cga");
	}
...
void cga_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void cga_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

cga_qt::settings::settings (cga_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
wallet (wallet_a)
{
	...
	new_representative->setPlaceholderText (cga::zero_key.pub.to_account ().c_str ());
...
void cga_qt::settings::refresh_representative ()
{
...
void cga_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void cga_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (cga_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (cga_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (cga_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (cga_qt::status_types::vulnerable);
	}
}

cga_qt::advanced_actions::advanced_actions (cga_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
mcga_unit (new QRadioButton ("Mcga")),
kcga_unit (new QRadioButton ("kcga")),
cga_unit (new QRadioButton ("cga")),
back (new QPushButton ("Back")),
...
bootstrap_label (new QLabel ("IPV6:port \"::ffff:192.168.0.1:7032\"")),
...
wallet (wallet_a)
{
	ratio_group->addButton (mcga_unit);
	ratio_group->addButton (kcga_unit);
	ratio_group->addButton (cga_unit);
	ratio_group->setId (mcga_unit, 0);
	ratio_group->setId (kcga_unit, 1);
	ratio_group->setId (cga_unit, 2);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mcga_unit);
	scale_layout->addWidget (kcga_unit);
	scale_layout->addWidget (cga_unit);
	...
	QObject::connect (mcga_unit, &QRadioButton::toggled, [this]() {
		if (mcga_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (mcga_unit));
			this->wallet.change_rendering_ratio (cga::Mcga_ratio);
		}
	});
	QObject::connect (kcga_unit, &QRadioButton::toggled, [this]() {
		if (kcga_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (kcga_unit));
			this->wallet.change_rendering_ratio (cga::kcga_ratio);
		}
	});
	QObject::connect (cga_unit, &QRadioButton::toggled, [this]() {
		if (cga_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (cga_unit));
			this->wallet.change_rendering_ratio (cga::cga_ratio);
		}
	});
	auto selected_ratio_id (QSettings ().value (saved_ratio_key, ratio_group->id (mcga_unit)).toInt ());
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this]() {
		...
		auto error (cga::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void cga_qt::advanced_actions::refresh_peers ()
{
...
void cga_qt::advanced_actions::refresh_ledger ()
{
	...
	{
		QList<QStandardItem *> items;
		items.push_back (new QStandardItem (QString (cga::block_hash (i->first).to_account ().c_str ())));
		cga::account_info info (i->second);
		std::string balance;
		cga::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void cga_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

cga_qt::block_entry::block_entry (cga_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this]() {
		...
		try
		{
			...
			auto block_l (cga::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
cga_qt::block_creation::block_creation (cga_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void cga_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void cga_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void cga_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void cga_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void cga_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void cga_qt::block_creation::create_send ()
{
	cga::account account_l;
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						cga::state_block send (account_l, info.head, rep_block->representative (), balance - amount_l.number (), destination_l, key, account_l, 0);
...
void cga_qt::block_creation::create_receive ()
{
	cga::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				cga::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							auto rep_block (wallet.node.store.block_get (block_transaction, info.rep_block));
							assert (rep_block != nullptr);
							cga::state_block receive (pending_key.account, info.head, rep_block->representative (), info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
...
void cga_qt::block_creation::create_change ()
{
	cga::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					cga::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
...
void cga_qt::block_creation::create_open ()
{
	cga::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								cga::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
