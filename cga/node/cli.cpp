...
std::string cga::error_cli_messages::message (int ev) const
{
	switch (static_cast<cga::error_cli> (ev))
	{
		case cga::error_cli::generic:
			return "Unknown error";
		case cga::error_cli::parse_error:
			return "Coud not parse command line";
		case cga::error_cli::invalid_arguments:
			return "Invalid arguments";
		case cga::error_cli::unknown_command:
...
void cga::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
std::error_code cga::handle_node_options (boost::program_options::variables_map & vm)
{
	std::error_code ec;

	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : cga::working_path ();
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = cga::error_cli::invalid_arguments;
					}
				}
				else
				{
	...
	else if (vm.count ("diagnostics"))
	{
		...
		cga::raw_key key;
		key.data.clear ();
		cga::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		cga::raw_key junk1;
		junk1.data.clear ();
		cga::uint256_union junk2 (0);
		cga::kdf kdf;
		kdf.phs (junk1, "", junk2);
				...
				if (existing != node.node->wallets.items.end ())
				{
					...
					if (!existing->second->enter_password (transaction, password))
					{
						...
						for (auto i (existing->second->store.begin (transaction)), m (existing->second->store.end ()); i != m; ++i)
						{
							...
							if (cga::pub_key (key.data) != account)
							{
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node.node->wallets.items.begin ()), n (node.node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << cga::uint256_union (j->first).to_account () << '\n';
			}
		}
	}
	...
	{
		ec = cga::error_cli::unknown_command;
	}
