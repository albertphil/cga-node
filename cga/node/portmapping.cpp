...
cga::port_mapping::port_mapping (cga::node & node_a) :
node (node_a),
devices (nullptr),
...
void cga::port_mapping::start ()
{
	check_mapping_loop ();
}

void cga::port_mapping::refresh_devices ()
{
...
void cga::port_mapping::refresh_mapping ()
{
	if (!cga::is_test_network)
	{
		std::lock_guard<std::mutex> lock (mutex);
...
int cga::port_mapping::check_mapping ()
{
	int result (3600);
	if (!cga::is_test_network)
	{
...
void cga::port_mapping::check_mapping_loop ()
{
