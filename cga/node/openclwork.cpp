...
__kernel void cga_work (__global ulong * attempt, __global ulong * result_a, __global uchar * item_a)
{
...
cga::opencl_environment::opencl_environment (bool & error_a)
{
...
void cga::opencl_environment::dump (std::ostream & stream)
{
	auto index (0);
...
cga::opencl_config::opencl_config () :
platform (0),
device (0),
threads (1024 * 1024)
{
...
cga::opencl_config::opencl_config (unsigned platform_a, unsigned device_a, unsigned threads_a) :
platform (platform_a),
device (device_a),
threads (threads_a)
{
...
cga::error cga::opencl_config::serialize_json (cga::jsonconfig & json) const
{
...
cga::error cga::opencl_config::deserialize_json (cga::jsonconfig & json)
{
...
cga::opencl_work::opencl_work (bool & error_a, cga::opencl_config const & config_a, cga::opencl_environment & environment_a, cga::logging & logging_a) :
config (config_a),
context (0),
attempt_buffer (0),
result_buffer (0),
...
logging (logging_a)
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			cga::random_pool::generate_block (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							size_t item_size (sizeof (cga::uint256_union));
							...
							if (!error_a)
							{
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										kernel = clCreateKernel (program, "cga_work", &kernel_error);
...
boost::optional<uint64_t> cga::opencl_work::generate_work (cga::uint256_union const & root_a)
{
	...
	while (cga::work_validate (root_a, result) && !error)
	{
		result = rand.next ();
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (cga::uint256_union), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<cga::opencl_work> cga::opencl_work::create (bool create_a, cga::opencl_config const & config_a, cga::logging & logging_a)
{
	std::unique_ptr<cga::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		if (!error)
		{
			result.reset (new cga::opencl_work (error, config_a, environment, logging_a));
			if (error)
			{
				result.reset ();
			}
