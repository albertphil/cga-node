...
cga::endpoint cga::map_endpoint_to_v6 (cga::endpoint const & endpoint_a)
{
	auto endpoint_l (endpoint_a);
	if (endpoint_l.address ().is_v4 ())
	{
		endpoint_l = cga::endpoint (boost::asio::ip::address_v6::v4_mapped (endpoint_l.address ().to_v4 ()), endpoint_l.port ());
	}
	return endpoint_l;
}

cga::peer_information::peer_information (cga::endpoint const & endpoint_a, unsigned network_version_a, boost::optional<cga::account> node_id_a) :
endpoint (endpoint_a),
...
cga::peer_information::peer_information (cga::endpoint const & endpoint_a, std::chrono::steady_clock::time_point const & last_contact_a, std::chrono::steady_clock::time_point const & last_attempt_a) :
endpoint (endpoint_a),
ip_address (endpoint_a.address ()),
...
bool cga::peer_information::operator< (cga::peer_information const & peer_information_a) const
{
	return endpoint < peer_information_a.endpoint;
}

cga::peer_container::peer_container (cga::endpoint const & self_a) :
self (self_a),
peer_observer ([](cga::endpoint const &) {}),
disconnect_observer ([]() {})
{
}

bool cga::peer_container::contacted (cga::endpoint const & endpoint_a, unsigned version_a)
{
	auto endpoint_l (cga::map_endpoint_to_v6 (endpoint_a));
	auto should_handshake (false);
	if (version_a < cga::node_id_version)
	{
	...
	else if (!known_peer (endpoint_l))
	{
		...
		if (peers.get<cga::peer_by_ip_addr> ().count (endpoint_l.address ()) < max_peers_per_ip)
		{
	...
	else
	{
		...
		{
			peers.modify (existing, [](cga::peer_information & info) {
...
bool cga::peer_container::known_peer (cga::endpoint const & endpoint_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (peers.find (endpoint_a));
...
std::deque<cga::endpoint> cga::peer_container::list_fanout ()
{
	auto peers (random_set (size_sqrt ()));
	std::deque<cga::endpoint> result;
...
std::deque<cga::endpoint> cga::peer_container::list ()
{
	std::deque<cga::endpoint> result;
	std::lock_guard<std::mutex> lock (mutex);
	...
	cga::random_pool::shuffle (result.begin (), result.end ());
	return result;
}

std::vector<cga::peer_information> cga::peer_container::list_vector (size_t count_a)
{
	...
	{
		result.resize (count_a, cga::peer_information (cga::endpoint{}, 0));
	}
	return result;
}

cga::endpoint cga::peer_container::bootstrap_peer ()
{
	cga::endpoint result (boost::asio::ip::address_v6::any (), 0);
	std::lock_guard<std::mutex> lock (mutex);
	;
	for (auto i (peers.get<4> ().begin ()), n (peers.get<4> ().end ()); i != n;)
	{
		...
		{
			result = i->endpoint;
			peers.get<4> ().modify (i, [](cga::peer_information & peer_a) {
...
boost::optional<cga::uint256_union> cga::peer_container::assign_syn_cookie (cga::endpoint const & endpoint)
{
...
bool cga::peer_container::validate_syn_cookie (cga::endpoint const & endpoint, cga::account node_id, cga::signature sig)
{
	auto ip_addr (endpoint.address ());
	assert (ip_addr.is_v6 ());
	std::unique_lock<std::mutex> lock (syn_cookie_mutex);
	...
	if (cookie_it != syn_cookies.end () && !cga::validate_message (node_id, cookie_it->second.cookie, sig))
	{
		result = false;
...
std::unordered_set<cga::endpoint> cga::peer_container::random_set (size_t count_a)
{
	std::unordered_set<cga::endpoint> result;
	...
	if (!peers.empty ())
	{
		...
		{
			auto index (cga::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (peers_size - 1)));
...
void cga::peer_container::random_fill (std::array<cga::endpoint, 8> & target_a)
{
	auto peers (random_set (target_a.size ()));
	assert (peers.size () <= target_a.size ());
	auto endpoint (cga::endpoint (boost::asio::ip::address_v6{}, 0));
...
std::vector<cga::peer_information> cga::peer_container::representatives (size_t count_a)
{
	std::vector<peer_information> result;
...
void cga::peer_container::purge_syn_cookies (std::chrono::steady_clock::time_point const & cutoff)
{
...
std::vector<cga::peer_information> cga::peer_container::purge_list (std::chrono::steady_clock::time_point const & cutoff)
{
	std::vector<cga::peer_information> result;
	{
		std::lock_guard<std::mutex> lock (mutex);
		...
		for (auto i (peers.begin ()), n (peers.end ()); i != n; ++i)
		{
			peers.modify (i, [](cga::peer_information & info) { info.last_attempt = std::chrono::steady_clock::now (); });
		}
...
std::vector<cga::endpoint> cga::peer_container::rep_crawl ()
{
	std::vector<cga::endpoint> result;
...
size_t cga::peer_container::size ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return peers.size ();
}

size_t cga::peer_container::size_sqrt ()
{
	return (static_cast<size_t> (std::ceil (std::sqrt (size ()))));
}

std::vector<cga::peer_information> cga::peer_container::list_probable_rep_weights ()
{
	std::vector<cga::peer_information> result;
...
cga::uint128_t cga::peer_container::total_weight ()
{
...
bool cga::peer_container::empty ()
{
	return size () == 0;
}

bool cga::peer_container::not_a_peer (cga::endpoint const & endpoint_a, bool blacklist_loopback)
{
	bool result (false);
	...
	else if (cga::reserved_address (endpoint_a, blacklist_loopback))
	{
		result = true;
	}
...
bool cga::peer_container::rep_response (cga::endpoint const & endpoint_a, cga::account const & rep_account_a, cga::amount const & weight_a)
{
	...
	if (existing != peers.end ())
	{
		peers.modify (existing, [weight_a, &updated, rep_account_a](cga::peer_information & info) {
...
void cga::peer_container::rep_request (cga::endpoint const & endpoint_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (peers.find (endpoint_a));
	if (existing != peers.end ())
...
bool cga::peer_container::reachout (cga::endpoint const & endpoint_a)
{
...
bool cga::peer_container::insert (cga::endpoint const & endpoint_a, unsigned version_a, bool preconfigured_a, boost::optional<cga::account> node_id_a)
{
	assert (endpoint_a.address ().is_v6 ());
	auto unknown (false);
	...
	if (!result)
	{
		if (version_a >= cga::protocol_version_min)
		{
			std::lock_guard<std::mutex> lock (mutex);
			auto existing (peers.find (endpoint_a));
			if (existing != peers.end ())
			{
				peers.modify (existing, [node_id_a](cga::peer_information & info) {
					info.last_contact = std::chrono::steady_clock::now ();
			...
			{
				unknown = true;
				if (!result && !cga::is_test_network)
				{
					auto ip_peers (peers.get<cga::peer_by_ip_addr> ().count (endpoint_a.address ()));
				...
				if (!result)
				{
					peers.insert (cga::peer_information (endpoint_a, version_a, node_id_a));
				}
			}
		}
	}
