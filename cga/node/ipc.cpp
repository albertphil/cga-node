...
cga::error cga::ipc::ipc_config::serialize_json (cga::jsonconfig & json) const
{
	cga::jsonconfig tcp_l;
...
cga::error cga::ipc::ipc_config::deserialize_json (cga::jsonconfig & json)
{
...
{
public:
	session (cga::ipc::ipc_server & server_a, boost::asio::io_context & io_ctx_a, cga::ipc::ipc_config_transport & config_transport_a) :
	...
	void rpc_handle_query ()
	{
		...
		// This is called when cga::rpc_handler#process_request is done. We convert to
		...
		node.stats.inc (cga::stat::type::ipc, cga::stat::detail::invocations);
		...
		auto handler (std::make_shared<cga::rpc_handler> (node, server.rpc, body, request_id_l, response_handler_l));
	...
	void read_next_request ()
	{
		...
		async_read_exactly (buffer.data (), buffer.size (), std::chrono::seconds::max (), [this_l]() {
			...
			else if (this_l->buffer[preamble_offset::encoding] == static_cast<uint8_t> (cga::ipc::payload_encoding::json_legacy))
			{
				// Length of payload
...
private:
	...
	cga::timer<std::chrono::microseconds> session_timer;
	...
	cga::ipc::ipc_config_transport & config_transport;
};

/** Domain and TCP socket transport */
template <typename ACCEPTOR_TYPE, typename SOCKET_TYPE, typename ENDPOINT_TYPE>
class socket_transport : public cga::ipc::transport
{
public:
	socket_transport (cga::ipc::ipc_server & server_a, ENDPOINT_TYPE endpoint_a, cga::ipc::ipc_config_transport & config_transport_a, int concurrency_a) :
	server (server_a), config_transport (config_transport_a)
	{
		...
		if (concurrency_a > 0)
		{
			runner = std::make_unique<cga::thread_runner> (*io_ctx, concurrency_a);
		}
	}
...
class cga::ipc::dsock_file_remover final
{
public:
...
cga::ipc::ipc_server::ipc_server (cga::node & node_a, cga::rpc & rpc_a) :
...
cga::ipc::ipc_server::~ipc_server ()
{
	BOOST_LOG (node.log) << "IPC: server stopped";
}

void cga::ipc::ipc_server::stop ()
{
 ...
 */
class client_impl : public cga::ipc::ipc_client_impl
{
public:
	...
	void connect (std::string const & host_a, uint16_t port_a, std::function<void(cga::error)> callback_a)
	{
		...
		tcp_client->async_resolve (host_a, port_a, [this, callback_a](boost::system::error_code const & ec_resolve_a, boost::asio::ip::tcp::endpoint endpoint_a) {
			if (!ec_resolve_a)
			{
				this->tcp_client->async_connect ([callback_a](const boost::system::error_code & ec_connect_a) {
					callback_a (cga::error (ec_connect_a));
				});
			}
			else
			{
				callback_a (cga::error (ec_resolve_a));
			}
		});
	}

	cga::error connect (std::string const & path_a)
	{
		cga::error err;
#if defined(BOOST_ASIO_HAS_LOCAL_SOCKETS)
...
#else
		err = cga::error ("Domain sockets are not supported by this platform");
#endif
		return err;
	}
...
cga::ipc::ipc_client::ipc_client (boost::asio::io_context & io_ctx_a) :
io_ctx (io_ctx_a)
{
}

cga::error cga::ipc::ipc_client::connect (std::string const & path_a)
{
	impl = std::make_unique<client_impl> (io_ctx);
	return boost::polymorphic_downcast<client_impl *> (impl.get ())->connect (path_a);
}

void cga::ipc::ipc_client::async_connect (std::string const & host_a, uint16_t port_a, std::function<void(cga::error)> callback_a)
{
	impl = std::make_unique<client_impl> (io_ctx);
...
cga::error cga::ipc::ipc_client::connect (std::string const & host, uint16_t port)
{
	std::promise<cga::error> result_l;
	async_connect (host, port, [&result_l](cga::error err_a) {
		result_l.set_value (err_a);
	});
	return result_l.get_future ().get ();
}

void cga::ipc::ipc_client::async_write (std::shared_ptr<std::vector<uint8_t>> buffer_a, std::function<void(cga::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
	client->get_channel ().async_write (buffer_a, [callback_a](const boost::system::error_code & ec_a, size_t bytes_written_a) {
		callback_a (cga::error (ec_a), bytes_written_a);
	});
}

void cga::ipc::ipc_client::async_read (std::shared_ptr<std::vector<uint8_t>> buffer_a, size_t size_a, std::function<void(cga::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
	client->get_channel ().async_read (buffer_a, size_a, [callback_a](const boost::system::error_code & ec_a, size_t bytes_read_a) {
		callback_a (cga::error (ec_a), bytes_read_a);
	});
}

std::shared_ptr<std::vector<uint8_t>> cga::ipc::ipc_client::prepare_request (cga::ipc::payload_encoding encoding_a, std::string const & payload_a)
{
	auto buffer_l (std::make_shared<std::vector<uint8_t>> ());
	if (encoding_a == cga::ipc::payload_encoding::json_legacy)
	{
...
std::string cga::ipc::rpc_ipc_client::request (std::string const & rpc_action_a)
{
	auto req (prepare_request (cga::ipc::payload_encoding::json_legacy, rpc_action_a));
	auto res (std::make_shared<std::vector<uint8_t>> ());

	std::promise<std::string> result_l;
	async_write (req, [this, &res, &result_l](cga::error err_a, size_t size_a) {
		// Read length
		this->async_read (res, sizeof (uint32_t), [this, &res, &result_l](cga::error err_read_a, size_t size_read_a) {
			uint32_t payload_size_l = boost::endian::big_to_native (*reinterpret_cast<uint32_t *> (res->data ()));
			// Read json payload
			this->async_read (res, payload_size_l, [&res, &result_l](cga::error err_read_a, size_t size_read_a) {
