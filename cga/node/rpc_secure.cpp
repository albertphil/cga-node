...
bool cga::rpc_secure::on_verify_certificate (bool preverified, boost::asio::ssl::verify_context & ctx)
{
...
void cga::rpc_secure::load_certs (boost::asio::ssl::context & context_a)
{
...
cga::rpc_secure::rpc_secure (boost::asio::io_service & service_a, cga::node & node_a, cga::rpc_config const & config_a) :
...
void cga::rpc_secure::accept ()
{
	auto connection (std::make_shared<cga::rpc_connection_secure> (node, *this));
...
cga::rpc_connection_secure::rpc_connection_secure (cga::node & node_a, cga::rpc_secure & rpc_a) :
cga::rpc_connection (node_a, rpc_a),
stream (socket, rpc_a.ssl_context)
{
}

void cga::rpc_connection_secure::parse_connection ()
{
	// Perform the SSL handshake
	auto this_l = std::static_pointer_cast<cga::rpc_connection_secure> (shared_from_this ());
...
void cga::rpc_connection_secure::on_shutdown (const boost::system::error_code & error)
{
...
void cga::rpc_connection_secure::handle_handshake (const boost::system::error_code & error)
{
	if (!error)
	{
		read ();
...
void cga::rpc_connection_secure::read ()
{
	auto this_l (std::static_pointer_cast<cga::rpc_connection_secure> (shared_from_this ()));
				...
				switch (method)
				{
					...
					case boost::beast::http::verb::options:
					{
						...
						boost::beast::http::async_write (this_l->stream, this_l->res, [this_l](boost::system::error_code const & ec, size_t bytes_transferred) {
							...
							std::bind (
							&cga::rpc_connection_secure::on_shutdown,
							this_l,
							std::placeholders::_1));
