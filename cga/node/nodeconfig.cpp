...
// once cga::network is factored out of node.{c|h}pp
...
node_config (cga::network::node_port, cga::logging ())
{
}

cga::node_config::node_config (uint16_t peering_port_a, cga::logging const & logging_a) :
peering_port (peering_port_a),
logging (logging_a),
bootstrap_fraction_numerator (1),
receive_minimum (1000),
vote_minimum (cga::Gcga_ratio),
online_weight_minimum (60000 * cga::Gcga_ratio),
...
unchecked_cutoff_time (std::chrono::seconds (4 * 60 * 60)) // 4 hours
{
	...
	epoch_block_signer = cga::genesis_account;
	switch (cga::cga_network)
	{

		//미리 설정되는 대리자의 public key
...
cga::error cga::node_config::serialize_json (cga::jsonconfig & json) const
{
	...
	cga::jsonconfig preconfigured_representatives_l;
...
bool cga::node_config::upgrade_json (unsigned version_a, cga::jsonconfig & json)
{
	json.put ("version", json_version ());
	auto upgraded (false);
	switch (version_a)
	{
		...
		case 2:
		{
			json.put ("inactive_supply", cga::uint128_union (0).to_string_dec ());
		...
		case 3:
			...
			json.put ("receive_minimum", cga::cga_ratio.convert_to<std::string> ());
			upgraded = true;
		case 4:
			json.erase ("receive_minimum");
			json.put ("receive_minimum", cga::cga_ratio.convert_to<std::string> ());
		...
		case 8:
			...
			upgraded = true;
		case 9:
			json.put ("state_block_parse_canary", cga::block_hash (0).to_string ());
			json.put ("state_block_generate_canary", cga::block_hash (0).to_string ());
			upgraded = true;
		case 10:
			json.put ("online_weight_minimum", online_weight_minimum.to_string_dec ());
...
cga::error cga::node_config::deserialize_json (bool & upgraded_a, cga::jsonconfig & json)
{
	try
	{
...
cga::account cga::node_config::random_representative ()
{
	assert (preconfigured_representatives.size () > 0);
	size_t index (cga::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (preconfigured_representatives.size () - 1)));
	auto result (preconfigured_representatives[index]);
	return result;
}

cga::node_flags::node_flags () :
