...
size_t constexpr cga::frontier_req_client::size_frontier;

cga::socket::socket (std::shared_ptr<cga::node> node_a) :
...
void cga::socket::async_connect (cga::tcp_endpoint const & endpoint_a, std::function<void(boost::system::error_code const &)> callback_a)
{
	checkup ();
...
void cga::socket::async_read (std::shared_ptr<std::vector<uint8_t>> buffer_a, size_t size_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	...
	if (socket_m.is_open ())
	{
		...
		boost::asio::async_read (socket_m, boost::asio::buffer (buffer_a->data (), size_a), [this_l, callback_a](boost::system::error_code const & ec, size_t size_a) {
			this_l->node->stats.add (cga::stat::type::traffic_bootstrap, cga::stat::dir::in, size_a);
			this_l->stop ();
...
void cga::socket::async_write (std::shared_ptr<std::vector<uint8_t>> buffer_a, std::function<void(boost::system::error_code const &, size_t)> callback_a)
{
	auto this_l (shared_from_this ());
	if (socket_m.is_open ())
	{
		...
		boost::asio::async_write (socket_m, boost::asio::buffer (buffer_a->data (), buffer_a->size ()), [this_l, callback_a, buffer_a](boost::system::error_code const & ec, size_t size_a) {
			this_l->node->stats.add (cga::stat::type::traffic_bootstrap, cga::stat::dir::out, size_a);
			this_l->stop ();
			callback_a (ec, size_a);
		});
	}
...
void cga::socket::start (std::chrono::steady_clock::time_point timeout_a)
{
	cutoff = timeout_a.time_since_epoch ().count ();
}

void cga::socket::stop ()
{
	cutoff = std::numeric_limits<uint64_t>::max ();
}

void cga::socket::close ()
{
	if (socket_m.is_open ())
	{
		try
...
void cga::socket::checkup ()
{
	std::weak_ptr<cga::socket> this_w (shared_from_this ());
...
cga::tcp_endpoint cga::socket::remote_endpoint ()
{
...
cga::bootstrap_client::bootstrap_client (std::shared_ptr<cga::node> node_a, std::shared_ptr<cga::bootstrap_attempt> attempt_a, cga::tcp_endpoint const & endpoint_a) :
node (node_a),
attempt (attempt_a),
socket (std::make_shared<cga::socket> (node_a)),
...
cga::bootstrap_client::~bootstrap_client ()
{
	--attempt->connections;
}

double cga::bootstrap_client::block_rate () const
{
...
void cga::frontier_req_client::run ()
{
	std::unique_ptr<cga::frontier_req> request (new cga::frontier_req);
	request->start.clear ();
	...
	{
		cga::vectorstream stream (*send_buffer);
...
std::shared_ptr<cga::bootstrap_client> cga::bootstrap_client::shared ()
{
	return shared_from_this ();
}

cga::frontier_req_client::frontier_req_client (std::shared_ptr<cga::bootstrap_client> connection_a) :
...
cga::frontier_req_client::~frontier_req_client ()
{
}

void cga::frontier_req_client::receive_frontier ()
{
	auto this_l (shared_from_this ());
	connection->socket->async_read (connection->receive_buffer, cga::frontier_req_client::size_frontier, [this_l](boost::system::error_code const & ec, size_t size_a) {
		// An issue with asio is that sometimes, instead of reporting a bad file descriptor during disconnect,
		// we simply get a size of 0.
		if (size_a == cga::frontier_req_client::size_frontier)
		{
		...
		else
		{
			...
			{
				BOOST_LOG (this_l->connection->node->log) << boost::str (boost::format ("Invalid size: expected %1%, got %2%") % cga::frontier_req_client::size_frontier % size_a);
			}
		}
	});
}

void cga::frontier_req_client::unsynced (cga::block_hash const & head, cga::block_hash const & end)
{
...
void cga::frontier_req_client::received_frontier (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == cga::frontier_req_client::size_frontier);
		cga::account account;
		cga::bufferstream account_stream (connection->receive_buffer->data (), sizeof (account));
		auto error1 (cga::try_read (account_stream, account));
		assert (!error1);
		cga::block_hash latest;
		cga::bufferstream latest_stream (connection->receive_buffer->data () + sizeof (account), sizeof (latest));
		auto error2 (cga::try_read (latest_stream, latest));
		assert (!error2);
		if (count == 0)
		{
			start_time = std::chrono::steady_clock::now ();
			...
			if (!current.is_zero ())
			{
				if (account == current)
				{
					...
					else
					{
						...
						{
							connection->attempt->add_pull (cga::pull_info (account, latest, frontier));
				...
				else
				{
					...
					connection->attempt->add_pull (cga::pull_info (account, latest, cga::block_hash (0)));
				}
			}
			else
			{
...
void cga::frontier_req_client::next (cga::transaction const & transaction_a)
{
	...
	if (accounts.empty ())
	{
		...
		{
			cga::account_info info (i->second);
			accounts.push_back (std::make_pair (cga::account (i->first), info.head));
		}
		...
		{
			accounts.push_back (std::make_pair (cga::account (0), cga::block_hash (0)));
		}
	}
...
cga::bulk_pull_client::bulk_pull_client (std::shared_ptr<cga::bootstrap_client> connection_a, cga::pull_info const & pull_a) :
connection (connection_a),
known_account (0),
pull (pull_a),
total_blocks (0),
...
cga::bulk_pull_client::~bulk_pull_client ()
{
	// If received end block is not expected end block
	if (expected != pull.end)
	{
		pull.head = expected;
		if (connection->attempt->mode != cga::bootstrap_mode::legacy)
		{
...
void cga::bulk_pull_client::request ()
{
	expected = pull.head;
	cga::bulk_pull req;
...
void cga::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
...
void cga::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	cga::block_type type (static_cast<cga::block_type> (connection->receive_buffer->data ()[0]));
	switch (type)
	{
		case cga::block_type::send:
		{
			connection->socket->async_read (connection->receive_buffer, cga::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
		...
		{
			connection->socket->async_read (connection->receive_buffer, cga::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, cga::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, cga::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			connection->socket->async_read (connection->receive_buffer, cga::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void cga::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, cga::block_type type_a)
{
	if (!ec)
	{
		cga::bufferstream stream (connection->receive_buffer->data (), size_a);
		std::shared_ptr<cga::block> block (cga::deserialize_block (stream, type_a));
		if (block != nullptr && !cga::work_validate (*block))
		{
			...
			if (!stop_pull && !connection->hard_stop.load ())
			{
				...
				to prevent spam */
				if (connection->attempt->mode != cga::bootstrap_mode::legacy || unexpected_count < 16384)
				{
					receive_block ();
				}
			}
...
cga::bulk_push_client::bulk_push_client (std::shared_ptr<cga::bootstrap_client> const & connection_a) :
connection (connection_a)
{
}

cga::bulk_push_client::~bulk_push_client ()
{
}

void cga::bulk_push_client::start ()
{
	cga::bulk_push message;
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		cga::vectorstream stream (*buffer);
...
void cga::bulk_push_client::push (cga::transaction const & transaction_a)
{
	...
	while (block == nullptr && !finished)
	{
		...
		if (!finished)
		{
			...
			if (block == nullptr)
			{
				current_target.first = cga::block_hash (0);
			}
			else
			{
				if (connection->node->config.logging.bulk_pull_logging ())
...
void cga::bulk_push_client::send_finished ()
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	buffer->push_back (static_cast<uint8_t> (cga::block_type::not_a_block));
	connection->node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::bulk_push, cga::stat::dir::out);
	if (connection->node->config.logging.network_logging ())
	{
...
void cga::bulk_push_client::push_block (cga::block const & block_a)
{
	auto buffer (std::make_shared<std::vector<uint8_t>> ());
	{
		cga::vectorstream stream (*buffer);
		cga::serialize_block (stream, block_a);
	}
	auto this_l (shared_from_this ());
	connection->socket->async_write (buffer, [this_l](boost::system::error_code const & ec, size_t size_a) {
		if (!ec)
...
cga::bulk_pull_account_client::bulk_pull_account_client (std::shared_ptr<cga::bootstrap_client> connection_a, cga::account const & account_a) :
connection (connection_a),
account (account_a),
total_blocks (0)
{
...
cga::bulk_pull_account_client::~bulk_pull_account_client ()
{
	{
		std::lock_guard<std::mutex> mutex (connection->attempt->mutex);
		--connection->attempt->pulling;
...
void cga::bulk_pull_account_client::request ()
{
	cga::bulk_pull_account req;
	req.account = account;
	req.minimum_amount = connection->node->config.receive_minimum;
	req.flags = cga::bulk_pull_account_flags::pending_hash_and_amount;
...
void cga::bulk_pull_account_client::receive_pending ()
{
	auto this_l (shared_from_this ());
	size_t size_l (sizeof (cga::uint256_union) + sizeof (cga::uint128_union));
	connection->socket->async_read (connection->receive_buffer, size_l, [this_l, size_l](boost::system::error_code const & ec, size_t size_a) {
		...
		if (size_a == size_l)
		{
			if (!ec)
			{
				cga::block_hash pending;
				cga::bufferstream frontier_stream (this_l->connection->receive_buffer->data (), sizeof (cga::uint256_union));
				auto error1 (cga::try_read (frontier_stream, pending));
				assert (!error1);
				cga::amount balance;
				cga::bufferstream balance_stream (this_l->connection->receive_buffer->data () + sizeof (cga::uint256_union), sizeof (cga::uint128_union));
				auto error2 (cga::try_read (balance_stream, balance));
				assert (!error2);
...
cga::pull_info::pull_info (cga::account const & account_a, cga::block_hash const & head_a, cga::block_hash const & end_a, count_t count_a) :
account (account_a),
head (head_a),
end (end_a),
count (count_a),
...
cga::bootstrap_attempt::bootstrap_attempt (std::shared_ptr<cga::node> node_a) :
...
runs_count (0),
stopped (false),
mode (cga::bootstrap_mode::legacy),
lazy_stopped (0)
{
...
cga::bootstrap_attempt::~bootstrap_attempt ()
{
	BOOST_LOG (node->log) << "Exiting bootstrap attempt";
	node->bootstrap_initiator.notify_listeners (false);
}

bool cga::bootstrap_attempt::should_log ()
{
...
bool cga::bootstrap_attempt::request_frontier (std::unique_lock<std::mutex> & lock_a)
{
	auto result (true);
	...
	if (connection_l)
	{
		...
		{
			auto client (std::make_shared<cga::frontier_req_client> (connection_l));
			client->run ();
			frontiers = client;
...
void cga::bootstrap_attempt::request_pull (std::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		...
		pulls.pop_front ();
		if (mode != cga::bootstrap_mode::legacy)
		{
		...
		node->background ([connection_l, pull]() {
			auto client (std::make_shared<cga::bulk_pull_client> (connection_l, pull));
			client->request ();
		});
	}
}

void cga::bootstrap_attempt::request_push (std::unique_lock<std::mutex> & lock_a)
{
	bool error (false);
	if (auto connection_shared = connection_frontier_request.lock ())
	{
		std::future<bool> future;
		{
			auto client (std::make_shared<cga::bulk_push_client> (connection_shared));
			client->start ();
			push = client;
			future = client->promise.get_future ();
		}
...
bool cga::bootstrap_attempt::still_pulling ()
{
...
void cga::bootstrap_attempt::run ()
{
	...
	if (!pulls.empty ())
	{
		...
		{
			auto k = cga::random_pool::generate_word32 (0, i);
	...
	if (!stopped)
	{
		...
		{
			lock.unlock ();
			mode = cga::bootstrap_mode::wallet_lazy;
			wallet_run ();
			lock.lock ();
		}
...
std::shared_ptr<cga::bootstrap_client> cga::bootstrap_attempt::connection (std::unique_lock<std::mutex> & lock_a)
{
	while (!stopped && idle.empty ())
	{
		condition.wait (lock_a);
	}
	std::shared_ptr<cga::bootstrap_client> result;
	if (!idle.empty ())
	{
		result = idle.back ();
		idle.pop_back ();
...
bool cga::bootstrap_attempt::consume_future (std::future<bool> & future_a)
{
	bool result;
	try
	{
...
{
	bool operator() (const std::shared_ptr<cga::bootstrap_client> & lhs, const std::shared_ptr<cga::bootstrap_client> & rhs) const
	{
		return lhs->block_rate () > rhs->block_rate ();
	}
};

unsigned cga::bootstrap_attempt::target_connections (size_t pulls_remaining)
{
...
void cga::bootstrap_attempt::populate_connections ()
{
	double rate_sum = 0.0;
	size_t num_pulls = 0;
	std::priority_queue<std::shared_ptr<cga::bootstrap_client>, std::vector<std::shared_ptr<cga::bootstrap_client>>, block_rate_cmp> sorted_connections;
	std::unordered_set<cga::tcp_endpoint> endpoints;
	{
		std::unique_lock<std::mutex> lock (mutex);
		num_pulls = pulls.size ();
		std::deque<std::weak_ptr<cga::bootstrap_client>> new_clients;
		for (auto & c : clients)
		{
	...
	if (connections < target)
	{
		...
		for (auto i = 0u; i < delta; i++)
		{
			...
			auto endpoint (cga::tcp_endpoint (peer.address (), peer.port ()));
			if (peer != cga::endpoint (boost::asio::ip::address_v6::any (), 0) && endpoints.find (endpoint) == endpoints.end ())
			{
				auto client (std::make_shared<cga::bootstrap_client> (node, shared_from_this (), endpoint));
				client->run ();
				std::lock_guard<std::mutex> lock (mutex);
	...
	if (!stopped)
	{
		std::weak_ptr<cga::bootstrap_attempt> this_w (shared_from_this ());
...
void cga::bootstrap_attempt::add_connection (cga::endpoint const & endpoint_a)
{
	auto client (std::make_shared<cga::bootstrap_client> (node, shared_from_this (), cga::tcp_endpoint (endpoint_a.address (), endpoint_a.port ())));
	client->run ();
}

void cga::bootstrap_attempt::pool_connection (std::shared_ptr<cga::bootstrap_client> client_a)
{
	{
		std::lock_guard<std::mutex> lock (mutex);
...
void cga::bootstrap_attempt::stop ()
{
	std::lock_guard<std::mutex> lock (mutex);
	stopped = true;
	condition.notify_all ();
...
void cga::bootstrap_attempt::add_pull (cga::pull_info const & pull)
{
	{
		std::lock_guard<std::mutex> lock (mutex);
...
void cga::bootstrap_attempt::requeue_pull (cga::pull_info const & pull_a)
{
	auto pull (pull_a);
	...
	else if (mode == cga::bootstrap_mode::lazy)
	{
		{
...
void cga::bootstrap_attempt::add_bulk_push_target (cga::block_hash const & head, cga::block_hash const & end)
{
	std::lock_guard<std::mutex> lock (mutex);
	bulk_push_targets.push_back (std::make_pair (head, end));
}

void cga::bootstrap_attempt::lazy_start (cga::block_hash const & hash_a)
{
	std::unique_lock<std::mutex> lock (lazy_mutex);
...
void cga::bootstrap_attempt::lazy_add (cga::block_hash const & hash_a)
{
...
void cga::bootstrap_attempt::lazy_pull_flush ()
{
	assert (!mutex.try_lock ());
	...
	for (auto & pull_start : lazy_pulls)
	{
		...
		{
			pulls.push_back (cga::pull_info (pull_start, pull_start, cga::block_hash (0), lazy_max_pull_blocks));
		}
	}
	lazy_pulls.clear ();
}

bool cga::bootstrap_attempt::lazy_finished ()
{
	bool result (true);
	auto transaction (node->store.tx_begin_read ());
	std::unique_lock<std::mutex> lock (lazy_mutex);
...
void cga::bootstrap_attempt::lazy_clear ()
{
	assert (!lazy_mutex.try_lock ());
	lazy_blocks.clear ();
	lazy_keys.clear ();
...
void cga::bootstrap_attempt::lazy_run ()
{
	populate_connections ();
...
bool cga::bootstrap_attempt::process_block (std::shared_ptr<cga::block> block_a, cga::account const & known_account_a, uint64_t total_blocks, bool block_expected)
{
	bool stop_pull (false);
	if (mode != cga::bootstrap_mode::legacy && block_expected)
	{
		...
		if (lazy_blocks.find (hash) == lazy_blocks.end ())
		{
			...
			{
				cga::uint128_t balance (std::numeric_limits<cga::uint128_t>::max ());
				cga::unchecked_info info (block_a, known_account_a, 0, cga::signature_verification::unknown);
				...
				else if (block_a->type () == cga::block_type::send)
				{
					// Calculate balance for legacy send blocks
					std::shared_ptr<cga::send_block> block_l (std::static_pointer_cast<cga::send_block> (block_a));
					if (block_l != nullptr)
					{
						balance = block_l->hashables.balance.number ();
					}
				}
				else if (block_a->type () == cga::block_type::state)
				{
					std::shared_ptr<cga::state_block> block_l (std::static_pointer_cast<cga::state_block> (block_a));
					if (block_l != nullptr)
					{
						balance = block_l->hashables.balance.number ();
						cga::block_hash link (block_l->hashables.link);
						...
						{
							cga::block_hash previous (block_l->hashables.previous);
							...
							{
								cga::amount prev_balance (node->ledger.balance (transaction, previous));
			...
			if (find_state != lazy_state_unknown.end ())
			{
				...
				if (block_a->type () == cga::block_type::state)
				{
					std::shared_ptr<cga::state_block> block_l (std::static_pointer_cast<cga::state_block> (block_a));
	...
	else if (mode != cga::bootstrap_mode::legacy)
	{
...
void cga::bootstrap_attempt::request_pending (std::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		...
		node->background ([connection_l, account]() {
			auto client (std::make_shared<cga::bulk_pull_account_client> (connection_l, account));
			client->request ();
		});
	}
}

void cga::bootstrap_attempt::requeue_pending (cga::account const & account_a)
{
...
void cga::bootstrap_attempt::wallet_start (std::deque<cga::account> & accounts_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	wallet_accounts.swap (accounts_a);
}

bool cga::bootstrap_attempt::wallet_finished ()
{
	assert (!mutex.try_lock ());
	auto running (!stopped);
...
void cga::bootstrap_attempt::wallet_run ()
{
	populate_connections ();
	auto start_time (std::chrono::steady_clock::now ());
...
cga::bootstrap_initiator::bootstrap_initiator (cga::node & node_a) :
node (node_a),
stopped (false),
thread ([this]() {
	cga::thread_role::set (cga::thread_role::name::bootstrap_initiator);
	run_bootstrap ();
})
{
}
...
void cga::bootstrap_initiator::bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped && attempt == nullptr)
	{
		node.stats.inc (cga::stat::type::bootstrap, cga::stat::detail::initiate, cga::stat::dir::out);
		attempt = std::make_shared<cga::bootstrap_attempt> (node.shared ());
		condition.notify_all ();
	}
}

void cga::bootstrap_initiator::bootstrap (cga::endpoint const & endpoint_a, bool add_to_peers)
{
	if (add_to_peers)
	{
		node.peers.insert (cga::map_endpoint_to_v6 (endpoint_a), cga::protocol_version);
	}
	std::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
...
void cga::bootstrap_initiator::bootstrap_lazy (cga::block_hash const & hash_a, bool force)
{
	{
		std::unique_lock<std::mutex> lock (mutex);
		if (force)
		...
		node.stats.inc (cga::stat::type::bootstrap, cga::stat::detail::initiate_lazy, cga::stat::dir::out);
		if (attempt == nullptr)
		{
			attempt = std::make_shared<cga::bootstrap_attempt> (node.shared ());
			attempt->mode = cga::bootstrap_mode::lazy;
		}
...
void cga::bootstrap_initiator::bootstrap_wallet (std::deque<cga::account> & accounts_a)
{
	{
		std::unique_lock<std::mutex> lock (mutex);
		node.stats.inc (cga::stat::type::bootstrap, cga::stat::detail::initiate_wallet_lazy, cga::stat::dir::out);
		if (attempt == nullptr)
		{
			attempt = std::make_shared<cga::bootstrap_attempt> (node.shared ());
			attempt->mode = cga::bootstrap_mode::wallet_lazy;
		}
...
void cga::bootstrap_initiator::run_bootstrap ()
{
	std::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
		...
		{
			lock.unlock ();
			if (attempt->mode == cga::bootstrap_mode::legacy)
			{
				attempt->run ();
			}
			else if (attempt->mode == cga::bootstrap_mode::lazy)
			{
				attempt->lazy_run ();
			}
			else
...
void cga::bootstrap_initiator::add_observer (std::function<void(bool)> const & observer_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	observers.push_back (observer_a);
}

bool cga::bootstrap_initiator::in_progress ()
{
	return current_attempt () != nullptr;
}

std::shared_ptr<cga::bootstrap_attempt> cga::bootstrap_initiator::current_attempt ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return attempt;
}

void cga::bootstrap_initiator::stop ()
{
	{
		std::unique_lock<std::mutex> lock (mutex);
		stopped = true;
...
void cga::bootstrap_initiator::notify_listeners (bool in_progress_a)
{
...
cga::bootstrap_listener::bootstrap_listener (boost::asio::io_context & io_ctx_a, uint16_t port_a, cga::node & node_a) :
acceptor (io_ctx_a),
...
void cga::bootstrap_listener::start ()
{
...
void cga::bootstrap_listener::stop ()
{
...
void cga::bootstrap_listener::accept_connection ()
{
	if (acceptor.is_open ())
	{
		if (connections.size () < node.config.bootstrap_connections_max)
		{
			auto socket (std::make_shared<cga::socket> (node.shared ()));
...
void cga::bootstrap_listener::accept_action (boost::system::error_code const & ec, std::shared_ptr<cga::socket> socket_a)
{
	if (!ec)
	{
		auto connection (std::make_shared<cga::bootstrap_server> (socket_a, node.shared ()));
		{
			std::lock_guard<std::mutex> lock (mutex);
			if (acceptor.is_open ())
			{
...
boost::asio::ip::tcp::endpoint cga::bootstrap_listener::endpoint ()
{
...
cga::bootstrap_server::~bootstrap_server ()
{
	if (node->config.logging.bulk_pull_logging ())
	{
...
cga::bootstrap_server::bootstrap_server (std::shared_ptr<cga::socket> socket_a, std::shared_ptr<cga::node> node_a) :
receive_buffer (std::make_shared<std::vector<uint8_t>> ()),
socket (socket_a),
node (node_a)
{
...
void cga::bootstrap_server::receive ()
{
	auto this_l (shared_from_this ());
...
void cga::bootstrap_server::receive_header_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 8);
		cga::bufferstream type_stream (receive_buffer->data (), size_a);
		auto error (false);
		cga::message_header header (error, type_stream);
		if (!error)
		{
			switch (header.type)
			{
				...
				{
					node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::bulk_pull, cga::stat::dir::in);
				...
				{
					node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::bulk_pull_account, cga::stat::dir::in);
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, header.payload_length_bytes (), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
				...
				{
					node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::frontier_req, cga::stat::dir::in);
					auto this_l (shared_from_this ());
					socket->async_read (receive_buffer, header.payload_length_bytes (), [this_l, header](boost::system::error_code const & ec, size_t size_a) {
				...
				{
					node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::bulk_push, cga::stat::dir::in);
					add_request (std::unique_ptr<cga::message> (new cga::bulk_push (header)));
					break;
				}
...
void cga::bootstrap_server::receive_bulk_pull_action (boost::system::error_code const & ec, size_t size_a, cga::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		cga::bufferstream stream (receive_buffer->data (), size_a);
		std::unique_ptr<cga::bulk_pull> request (new cga::bulk_pull (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
			...
			add_request (std::unique_ptr<cga::message> (request.release ()));
			receive ();
		}
	}
}

void cga::bootstrap_server::receive_bulk_pull_account_action (boost::system::error_code const & ec, size_t size_a, cga::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		...
		cga::bufferstream stream (receive_buffer->data (), size_a);
		std::unique_ptr<cga::bulk_pull_account> request (new cga::bulk_pull_account (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
				BOOST_LOG (node->log) << boost::str (boost::format ("Received bulk pull account for %1% with a minimum amount of %2%") % request->account.to_account () % cga::amount (request->minimum_amount).format_balance (cga::Mcga_ratio, 10, true));
			}
			add_request (std::unique_ptr<cga::message> (request.release ()));
			receive ();
		}
...
void cga::bootstrap_server::receive_keepalive_action (boost::system::error_code const & ec, size_t size_a, cga::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		cga::bufferstream stream (receive_buffer->data (), header_a.payload_length_bytes ());
		std::unique_ptr<cga::keepalive> request (new cga::keepalive (error, stream, header_a));
		if (!error)
		{
			add_request (std::unique_ptr<cga::message> (request.release ()));
			receive ();
...
void cga::bootstrap_server::receive_frontier_req_action (boost::system::error_code const & ec, size_t size_a, cga::message_header const & header_a)
{
	if (!ec)
	{
		auto error (false);
		cga::bufferstream stream (receive_buffer->data (), header_a.payload_length_bytes ());
		std::unique_ptr<cga::frontier_req> request (new cga::frontier_req (error, stream, header_a));
		if (!error)
		{
			if (node->config.logging.bulk_pull_logging ())
			{
...
void cga::bootstrap_server::add_request (std::unique_ptr<cga::message> message_a)
{
	std::lock_guard<std::mutex> lock (mutex);
...
void cga::bootstrap_server::finish_request ()
{
	std::lock_guard<std::mutex> lock (mutex);
	requests.pop ();
	if (!requests.empty ())
...
namespace
{
class request_response_visitor : public cga::message_visitor
{
public:
	request_response_visitor (std::shared_ptr<cga::bootstrap_server> connection_a) :
	connection (connection_a)
	{
	}
	...
	void keepalive (cga::keepalive const & message_a) override
	{
		...
		connection->node->stats.inc (cga::stat::type::message, cga::stat::detail::keepalive, cga::stat::dir::in);
		...
		connection->socket->async_write (bytes, [connection = connection](boost::system::error_code const & ec, size_t size_a) {
			...
			{
				connection->node->stats.inc (cga::stat::type::message, cga::stat::detail::keepalive, cga::stat::dir::out);
				connection->finish_request ();
			}
		});
	}
	void publish (cga::publish const &) override
	{
		assert (false);
	}
	void confirm_req (cga::confirm_req const &) override
	{
		assert (false);
	}
	void confirm_ack (cga::confirm_ack const &) override
	{
		assert (false);
	}
	void bulk_pull (cga::bulk_pull const &) override
	{
		auto response (std::make_shared<cga::bulk_pull_server> (connection, std::unique_ptr<cga::bulk_pull> (static_cast<cga::bulk_pull *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void bulk_pull_account (cga::bulk_pull_account const &) override
	{
		auto response (std::make_shared<cga::bulk_pull_account_server> (connection, std::unique_ptr<cga::bulk_pull_account> (static_cast<cga::bulk_pull_account *> (connection->requests.front ().release ()))));
		response->send_frontier ();
	}
	void bulk_push (cga::bulk_push const &) override
	{
		auto response (std::make_shared<cga::bulk_push_server> (connection));
		response->receive ();
	}
	void frontier_req (cga::frontier_req const &) override
	{
		auto response (std::make_shared<cga::frontier_req_server> (connection, std::unique_ptr<cga::frontier_req> (static_cast<cga::frontier_req *> (connection->requests.front ().release ()))));
		response->send_next ();
	}
	void node_id_handshake (cga::node_id_handshake const &) override
	{
		assert (false);
	}
...
void cga::bootstrap_server::run_next ()
{
 ...
 */
void cga::bulk_pull_server::set_current_end ()
{
	include_start = false;
...
void cga::bulk_pull_server::send_next ()
{
	auto block (get_next ());
	if (block != nullptr)
	{
		{
			send_buffer->clear ();
			cga::vectorstream stream (*send_buffer);
			cga::serialize_block (stream, *block);
		}
		auto this_l (shared_from_this ());
		if (connection->node->config.logging.bulk_pull_logging ())
		{
...
std::shared_ptr<cga::block> cga::bulk_pull_server::get_next ()
{
...
void cga::bulk_pull_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void cga::bulk_pull_server::send_finished ()
{
	send_buffer->clear ();
	send_buffer->push_back (static_cast<uint8_t> (cga::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
...
void cga::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
cga::bulk_pull_server::bulk_pull_server (std::shared_ptr<cga::bootstrap_server> const & connection_a, std::unique_ptr<cga::bulk_pull> request_a) :
connection (connection_a),
 ...
 */
void cga::bulk_pull_account_server::set_params ()
{
	assert (request != nullptr);
	...
	if (request->flags == cga::bulk_pull_account_flags::pending_address_only)
	{
		pending_address_only = true;
	}
	else if (request->flags == cga::bulk_pull_account_flags::pending_hash_amount_and_address)
	{
		/**
	...
	else if (request->flags == cga::bulk_pull_account_flags::pending_hash_and_amount)
	{
...
void cga::bulk_pull_account_server::send_frontier ()
{
	/*
	...
	cga::uint128_union account_frontier_balance (account_frontier_balance_int);
	...
	send_buffer->clear ();
	{
		cga::vectorstream output_stream (*send_buffer);
...
void cga::bulk_pull_account_server::send_next_block ()
{
	/*
...
std::pair<std::unique_ptr<cga::pending_key>, std::unique_ptr<cga::pending_info>> cga::bulk_pull_account_server::get_next ()
{
	std::pair<std::unique_ptr<cga::pending_key>, std::unique_ptr<cga::pending_info>> result;
	...
	while (true)
	{
		...
		if (stream == cga::store_iterator<cga::pending_key, cga::pending_info> (nullptr))
		{
			break;
		}

		cga::pending_key key (stream->first);
		cga::pending_info info (stream->second);
		...
		result.first = std::unique_ptr<cga::pending_key> (new cga::pending_key (key));
		result.second = std::unique_ptr<cga::pending_info> (new cga::pending_info (info));
...
void cga::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next_block ();
...
void cga::bulk_pull_account_server::send_finished ()
{
	/*
	...
	{
		cga::vectorstream output_stream (*send_buffer);
		cga::uint256_union account_zero (0);
		cga::uint128_union balance_zero (0);
...
void cga::bulk_pull_account_server::complete (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		if (pending_address_only)
...
cga::bulk_pull_account_server::bulk_pull_account_server (std::shared_ptr<cga::bootstrap_server> const & connection_a, std::unique_ptr<cga::bulk_pull_account> request_a) :
connection (connection_a),
request (std::move (request_a)),
send_buffer (std::make_shared<std::vector<uint8_t>> ()),
current_key (0, 0)
...
cga::bulk_push_server::bulk_push_server (std::shared_ptr<cga::bootstrap_server> const & connection_a) :
receive_buffer (std::make_shared<std::vector<uint8_t>> ()),
connection (connection_a)
{
	receive_buffer->resize (256);
...
void cga::bulk_push_server::receive ()
{
...
void cga::bulk_push_server::received_type ()
{
	auto this_l (shared_from_this ());
	cga::block_type type (static_cast<cga::block_type> (receive_buffer->data ()[0]));
	switch (type)
	{
		case cga::block_type::send:
		{
			connection->node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::send, cga::stat::dir::in);
			connection->socket->async_read (receive_buffer, cga::send_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case cga::block_type::receive:
		{
			connection->node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::receive, cga::stat::dir::in);
			connection->socket->async_read (receive_buffer, cga::receive_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case cga::block_type::open:
		{
			connection->node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::open, cga::stat::dir::in);
			connection->socket->async_read (receive_buffer, cga::open_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case cga::block_type::change:
		{
			connection->node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::change, cga::stat::dir::in);
			connection->socket->async_read (receive_buffer, cga::change_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		case cga::block_type::state:
		{
			connection->node->stats.inc (cga::stat::type::bootstrap, cga::stat::detail::state_block, cga::stat::dir::in);
			connection->socket->async_read (receive_buffer, cga::state_block::size, [this_l, type](boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void cga::bulk_push_server::received_block (boost::system::error_code const & ec, size_t size_a, cga::block_type type_a)
{
	if (!ec)
	{
		cga::bufferstream stream (receive_buffer->data (), size_a);
		auto block (cga::deserialize_block (stream, type_a));
		if (block != nullptr && !cga::work_validate (*block))
		{
...
cga::frontier_req_server::frontier_req_server (std::shared_ptr<cga::bootstrap_server> const & connection_a, std::unique_ptr<cga::frontier_req> request_a) :
connection (connection_a),
...
void cga::frontier_req_server::send_next ()
{
...
void cga::frontier_req_server::send_finished ()
{
	{
		send_buffer->clear ();
		cga::vectorstream stream (*send_buffer);
		cga::uint256_union zero (0);
...
void cga::frontier_req_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		connection->finish_request ();
...
void cga::frontier_req_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		count++;
...
void cga::frontier_req_server::next ()
{
	// Filling accounts deque to prevent often read transactions
	if (accounts.empty ())
	{
