...
#ifdef CGA_SECURE_RPC
#include <cga/node/rpc_secure.hpp>
#endif
...
namespace
{
void construct_json (cga::seq_con_info_component * component, boost::property_tree::ptree & parent);
}

cga::rpc_secure_config::rpc_secure_config () :
enable (false),
verbose_logging (false)
{
}

cga::error cga::rpc_secure_config::serialize_json (cga::jsonconfig & json) const
{
...
cga::error cga::rpc_secure_config::deserialize_json (cga::jsonconfig & json)
{
...
cga::rpc_config::rpc_config (bool enable_control_a) :
address (boost::asio::ip::address_v6::loopback ()),
port (cga::rpc::rpc_port),
...
cga::error cga::rpc_config::serialize_json (cga::jsonconfig & json) const
{
...
cga::error cga::rpc_config::deserialize_json (cga::jsonconfig & json)
{
...
cga::rpc::rpc (boost::asio::io_context & io_ctx_a, cga::node & node_a, cga::rpc_config const & config_a) :
acceptor (io_ctx_a),
config (config_a),
node (node_a)
{
...
void cga::rpc::add_block_observer ()
{
	node.observers.blocks.add ([this](std::shared_ptr<cga::block> block_a, cga::account const & account_a, cga::uint128_t const &, bool) {
		observer_action (account_a);
	});
}

void cga::rpc::start (bool rpc_enabled_a)
{
	if (rpc_enabled_a)
	{
		auto endpoint (cga::tcp_endpoint (config.address, config.port));
...
void cga::rpc::accept ()
{
	auto connection (std::make_shared<cga::rpc_connection> (node, *this));
...
cga::rpc_handler::rpc_handler (cga::node & node_a, cga::rpc & rpc_a, std::string const & body_a, std::string const & request_id_a, std::function<void(boost::property_tree::ptree const &)> const & response_a) :
body (body_a),
...
void cga::rpc::observer_action (cga::account const & account_a)
{
	std::shared_ptr<cga::payment_observer> observer;
	{
...
void cga::error_response (std::function<void(boost::property_tree::ptree const &)> response_a, std::string const & message_a)
{
...
void cga::rpc_handler::response_errors ()
{
...
std::shared_ptr<cga::wallet> cga::rpc_handler::wallet_impl ()
{
	if (!ec)
	{
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				ec = cga::error_common::wallet_not_found;
			}
		}
		else
		{
			ec = cga::error_common::bad_wallet_number;
		}
	}
	return nullptr;
}

bool cga::rpc_handler::wallet_locked_impl (cga::transaction const & transaction_a, std::shared_ptr<cga::wallet> wallet_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = cga::error_common::wallet_locked;
			result = true;
		}
	}
	return result;
...
bool cga::rpc_handler::wallet_account_impl (cga::transaction const & transaction_a, std::shared_ptr<cga::wallet> wallet_a, cga::account const & account_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = cga::error_common::account_not_found_wallet;
		}
	}
	return result;
}

cga::account cga::rpc_handler::account_impl (std::string account_text)
{
...
cga::amount cga::rpc_handler::amount_impl ()
{
	cga::amount result (0);
	if (!ec)
	{
		...
		{
			ec = cga::error_common::invalid_amount;
		}
	}
	return result;
}

std::shared_ptr<cga::block> cga::rpc_handler::block_impl (bool signature_work_required)
{
	...
	if (!ec)
	{
		...
		result = cga::deserialize_block_json (block_l);
		if (result == nullptr)
		{
			ec = cga::error_blocks::invalid_block;
		}
	}
	return result;
}

cga::block_hash cga::rpc_handler::hash_impl (std::string search_text)
{
	...
	if (!ec)
	{
		...
		{
			ec = cga::error_blocks::invalid_block_hash;
		}
	}
	return result;
}

cga::amount cga::rpc_handler::threshold_optional_impl ()
{
	cga::amount result (0);
	...
	if (!ec && threshold_text.is_initialized ())
	{
		...
		{
			ec = cga::error_common::bad_threshold;
		}
	}
	return result;
}

uint64_t cga::rpc_handler::work_optional_impl ()
{
	uint64_t result (0);
	boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
	if (!ec && work_text.is_initialized ())
	{
		if (cga::from_string_hex (work_text.get (), result))
		{
			ec = cga::error_common::bad_work_format;
		}
	}
	return result;
}
...
uint64_t cga::rpc_handler::count_impl ()
{
	uint64_t result (0);
	if (!ec)
	{
		...
		{
			ec = cga::error_common::invalid_count;
		}
	}
	return result;
}

uint64_t cga::rpc_handler::count_optional_impl (uint64_t result)
{
...
uint64_t cga::rpc_handler::offset_optional_impl (uint64_t result)
{
	...
	if (!ec && offset_text.is_initialized ())
	{
		...
		{
			ec = cga::error_rpc::invalid_offset;
		}
	}
	return result;
}

bool cga::rpc_handler::rpc_control_impl ()
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = cga::error_rpc::rpc_control_disabled;
		}
		else
		{
			result = true;
...
void cga::rpc_handler::account_balance ()
{
...
void cga::rpc_handler::account_block_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		{
			ec = cga::error_common::account_not_found;
		}
	}
	response_errors ();
}

void cga::rpc_handler::account_create ()
{
	rpc_control_impl ();
	...
	if (!ec)
	{
		...
		if (index_text.is_initialized ())
		{
			...
			{
				ec = cga::error_common::invalid_index;
			}
			else
			{
...
void cga::rpc_handler::account_get ()
{
	...
	{
		ec = cga::error_common::bad_public_key;
	}
	response_errors ();
}

void cga::rpc_handler::account_info ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (!node.store.account_get (transaction, account, info))
		{
			...
			std::string balance;
			cga::uint128_union (info.balance).encode_dec (balance);
			response_l.put ("balance", balance);
			response_l.put ("modified_timestamp", std::to_string (info.modified));
			response_l.put ("block_count", std::to_string (info.block_count));
			response_l.put ("account_version", info.epoch == cga::epoch::epoch_1 ? "1" : "0");
			if (representative)
			{
...
void cga::rpc_handler::account_key ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::account_list ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), j (wallet->store.end ()); i != j; ++i)
		{
			...
			entry.put ("", cga::account (i->first).to_account ());
...
void cga::rpc_handler::account_move ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		if (!source.decode_hex (source_text))
		{
			...
			if (existing != node.wallets.items.end ())
			{
				...
				std::vector<cga::public_key> accounts;
				for (auto i (accounts_text.begin ()), n (accounts_text.end ()); i != n; ++i)
				{
					cga::public_key account;
			...
			{
				ec = cga::error_rpc::source_not_found;
			}
		}
		else
		{
			ec = cga::error_rpc::bad_source;
		}
	}
	response_errors ();
}

void cga::rpc_handler::account_remove ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto account (account_impl ());
...
void cga::rpc_handler::account_representative ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::account_representative_set ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (!representative.decode_account (representative_text))
		{
			...
			if (!ec && work)
			{
				...
				if (!ec)
				{
					...
					{
						if (cga::work_validate (info.head, work))
						{
							ec = cga::error_common::invalid_work;
						}
					}
					else
					{
			...
			if (!ec)
			{
				...
				// clang-format off
				wallet->change_async (account, representative, [response_a](std::shared_ptr<cga::block> block) {
					cga::block_hash hash (0);
					if (block != nullptr)
					{
						hash = block->hash ();
					}
		...
		{
			ec = cga::error_rpc::bad_representative_number;
		}
	}
...
void cga::rpc_handler::account_weight ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::accounts_balances ()
{
...
void cga::rpc_handler::accounts_create ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	...
	if (!ec)
	{
		...
		{
			cga::account new_key (wallet->deterministic_insert (generate_work));
			if (!new_key.is_zero ())
			{
				boost::property_tree::ptree entry;
...
void cga::rpc_handler::accounts_frontiers ()
{
...
void cga::rpc_handler::accounts_pending ()
{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto i (node.store.pending_begin (transaction, cga::pending_key (account, 0))); cga::pending_key (i->first).account == account && peers_l.size () < count; ++i)
			{
				cga::pending_key key (i->first);
				std::shared_ptr<cga::block> block (include_active ? nullptr : node.store.block_get (transaction, key.hash));
...
void cga::rpc_handler::available_supply ()
{
	auto genesis_balance (node.balance (cga::genesis_account)); // Cold storage genesis
	auto landing_balance (node.balance (cga::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (cga::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (cga::account (0))).second); // Burning 0 account
	auto available (cga::genesis_amount - genesis_balance - landing_balance - faucet_balance - burned_balance);
	response_l.put ("available", available.convert_to<std::string> ());
	response_errors ();
}

void cga::rpc_handler::block_info ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			cga::account account (block->account ().is_zero () ? sideband.account : block->account ());
		...
		{
			ec = cga::error_blocks::not_found;
		}
	}
	response_errors ();
}

void cga::rpc_handler::block_confirm ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::blocks ()
{
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		if (!ec)
		{
			...
			{
				ec = cga::error_blocks::bad_hash_number;
			}
		}
	}
...
void cga::rpc_handler::blocks_info ()
{
	const bool pending = request.get<bool> ("pending", false);
	const bool source = request.get<bool> ("source", false);
	std::vector<std::string> hashes;
			...
			if (!hash.decode_hex (hash_text))
			{
				...
				if (block != nullptr)
				{
					...
					if (pending)
					{
						...
						{
							exists = node.store.pending_exists (transaction, cga::pending_key (destination, hash));
						}
						entry.put ("pending", exists ? "1" : "0");
					}
					if (source)
					{
						cga::block_hash source_hash (node.ledger.block_source (transaction, *block));
...
void cga::rpc_handler::block_account ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::block_count ()
{
	auto transaction (node.store.tx_begin_read ());
...
void cga::rpc_handler::block_count_type ()
{
	auto transaction (node.store.tx_begin_read ());
	cga::block_counts count (node.store.block_count (transaction));
...
void cga::rpc_handler::block_create ()
{
	rpc_control_impl ();
	if (!ec)
	{
		std::string type (request.get<std::string> ("type"));
		cga::uint256_union wallet (0);
		...
		cga::uint256_union representative (0);
		...
		cga::uint256_union destination (0);
		...
		if (!ec && destination_text.is_initialized ())
		{
			...
			{
				ec = cga::error_rpc::bad_destination;
			}
		}
		cga::block_hash source (0);
		...
		cga::raw_key prv;
		prv.data.clear ();
		cga::uint256_union previous (0);
		cga::uint128_union balance (0);
		...
		if (!ec && key_text.is_initialized ())
		{
			...
			{
				ec = cga::error_common::bad_private_key;
			}
		}
		...
		if (!ec && balance_text.is_initialized ())
		{
			...
			{
				ec = cga::error_rpc::invalid_balance;
			}
		}
		cga::uint256_union link (0);
		...
		if (prv.data != 0)
		{
			cga::uint256_union pub (cga::pub_key (prv.data));
			...
			else if (previous_text.is_initialized () && balance_text.is_initialized () && type == "send")
			{
				...
				{
					ec = cga::error_rpc::block_create_balance_mismatch;
				}
			}
			...
			if (!ec && account_text.is_initialized ())
			{
				if (account != pub)
				{
					ec = cga::error_rpc::block_create_public_key_mismatch;
				}
			}
			if (type == "state")
			{
				if (previous_text.is_initialized () && !representative.is_zero () && (!link.is_zero () || link_text.is_initialized ()))
				{
					...
					cga::state_block state (pub, previous, representative, balance, link, prv, pub, work);
				...
				{
					ec = cga::error_rpc::block_create_requirements_state;
				}
			}
			else if (type == "open")
			{
				if (representative != 0 && source != 0)
				{
					...
					cga::open_block open (source, representative, pub, prv, pub, work);
				...
				{
					ec = cga::error_rpc::block_create_requirements_open;
				}
			}
			else if (type == "receive")
			{
				if (source != 0 && previous != 0)
				{
					...
					cga::receive_block receive (previous, source, prv, pub, work);
				...
				{
					ec = cga::error_rpc::block_create_requirements_receive;
				}
			}
			else if (type == "change")
			{
				if (representative != 0 && previous != 0)
				{
					...
					cga::change_block change (previous, representative, prv, pub, work);
				...
				{
					ec = cga::error_rpc::block_create_requirements_change;
				}
			}
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					if (balance.number () >= amount.number ())
					{
						...
						cga::send_block send (previous, destination, balance.number () - amount.number (), prv, pub, work);
					...
					{
						ec = cga::error_common::insufficient_balance;
					}
				}
				else
				{
					ec = cga::error_rpc::block_create_requirements_send;
				}
			}
			else
			{
				ec = cga::error_blocks::invalid_type;
			}
		}
		else
		{
			ec = cga::error_rpc::block_create_key_required;
		}
	}
	response_errors ();
}

void cga::rpc_handler::block_hash ()
{
...
void cga::rpc_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!cga::parse_port (port_text, port))
		{
			node.bootstrap_initiator.bootstrap (cga::endpoint (address, port));
			response_l.put ("success", "");
		}
		else
		{
			ec = cga::error_common::invalid_port;
		}
	}
	else
	{
		ec = cga::error_common::invalid_ip_address;
	}
	response_errors ();
}

void cga::rpc_handler::bootstrap_any ()
{
...
void cga::rpc_handler::bootstrap_lazy ()
{
	rpc_control_impl ();
	auto hash (hash_impl ());
 ...
 */
void cga::rpc_handler::bootstrap_status ()
{
	...
	if (attempt != nullptr)
	{
		...
		else if (attempt->mode == cga::bootstrap_mode::wallet_lazy)
		{
...
void cga::rpc_handler::chain (bool successors)
{
...
void cga::rpc_handler::confirmation_active ()
{
...
void cga::rpc_handler::confirmation_history ()
{
	boost::property_tree::ptree elections;
...
void cga::rpc_handler::confirmation_info ()
{
	...
	if (!root.decode_hex (root_text))
	{
		...
		if (conflict_info != node.active.roots.end ())
		{
			...
			for (auto i (tally_l.begin ()), n (tally_l.end ()); i != n; ++i)
			{
				...
				if (representatives)
				{
					std::multimap<cga::uint128_t, cga::account, std::greater<cga::uint128_t>> representatives;
					for (auto ii (election->last_votes.begin ()), nn (election->last_votes.end ()); ii != nn; ++ii)
					{
						if (i->second->hash () == ii->second.hash)
						{
							cga::account representative (ii->first);
		...
		{
			ec = cga::error_rpc::confirmation_not_found;
		}
	}
	else
	{
		ec = cga::error_rpc::invalid_root;
	}
	response_errors ();
}

void cga::rpc_handler::confirmation_quorum ()
{
...
void cga::rpc_handler::delegators ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		for (auto i (node.store.latest_begin (transaction)), n (node.store.latest_end ()); i != n; ++i)
		{
			...
			if (block->representative () == account)
			{
				std::string balance;
				cga::uint128_union (info.balance).encode_dec (balance);
				delegators.put (cga::account (i->first).to_account (), balance);
			}
		}
...
void cga::rpc_handler::delegators_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::deterministic_key ()
{
	...
	if (!seed.data.decode_hex (seed_text))
	{
		try
		{
			...
			cga::uint256_union prv;
			cga::deterministic_key (seed.data, index, prv);
			cga::uint256_union pub (cga::pub_key (prv));
	...
	{
		ec = cga::error_common::bad_seed;
	}
	response_errors ();
}

void cga::rpc_handler::frontiers ()
{
	...
	if (!ec)
	{
		...
		{
			frontiers.put (cga::account (i->first).to_account (), cga::account_info (i->second).head.to_string ());
		}
		response_l.add_child ("frontiers", frontiers);
	}
	response_errors ();
...
void cga::rpc_handler::account_count ()
{
	auto transaction (node.store.tx_begin_read ());
...
namespace
{
class history_visitor : public cga::block_visitor
{
public:
	history_visitor (cga::rpc_handler & handler_a, bool raw_a, cga::transaction & transaction_a, boost::property_tree::ptree & tree_a, cga::block_hash const & hash_a) :
	handler (handler_a),
	raw (raw_a),
	...
	void send_block (cga::send_block const & block_a)
	{
		tree.put ("type", "send");
	...
	void receive_block (cga::receive_block const & block_a)
	{
	...
	void open_block (cga::open_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "open");
		...
		{
			tree.put ("account", cga::genesis_account.to_account ());
			tree.put ("amount", cga::genesis_amount.convert_to<std::string> ());
		}
	}
	void change_block (cga::change_block const & block_a)
	{
		if (raw)
		{
	...
	void state_block (cga::state_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "state");
	...
	cga::rpc_handler & handler;
	bool raw;
	cga::transaction & transaction;
	boost::property_tree::ptree & tree;
	cga::block_hash const & hash;
};
}

void cga::rpc_handler::account_history ()
{
	cga::account account;
		...
		while (block != nullptr && count > 0)
		{
			...
			else
			{
				...
				if (!entry.empty ())
				{
					...
					if (output_raw)
					{
						entry.put ("work", cga::to_string_hex (block->block_work ()));
...
void cga::rpc_handler::keepalive ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void cga::rpc_handler::key_create ()
{
	cga::keypair pair;
...
void cga::rpc_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	cga::uint256_union prv;
...
void cga::rpc_handler::ledger ()
{
	...
	if (!ec)
	{
		...
		if (modified_since_text.is_initialized ())
		{
			...
			{
				ec = cga::error_rpc::invalid_timestamp;
			}
		}
		...
		{
			std::vector<std::pair<cga::uint128_union, cga::account>> ledger_l;
			for (auto i (node.store.latest_begin (transaction, start)), n (node.store.latest_end ()); i != n; ++i)
			{
				cga::account_info info (i->second);
				cga::uint128_union balance (info.balance);
				if (info.modified >= modified_since)
				{
					ledger_l.push_back (std::make_pair (balance, cga::account (i->first)));
				}
			}
...
void cga::rpc_handler::mcga_from_raw (cga::uint128_t ratio)
{
...
void cga::rpc_handler::mcga_to_raw (cga::uint128_t ratio)
{
	auto amount (amount_impl ());
	if (!ec)
	{
		...
		{
			ec = cga::error_common::invalid_amount_big;
		}
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void cga::rpc_handler::node_id_delete ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void cga::rpc_handler::password_change ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void cga::rpc_handler::password_enter ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::password_valid (bool wallet_locked)
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
				...
				else
				{
					...
					if (info.amount.number () >= threshold.number ())
					{
						if (source || min_version)
						{
							...
							if (min_version)
							{
								pending_tree.put ("min_version", info.epoch == cga::epoch::epoch_1 ? "1" : "0");
							}
							peers_l.add_child (key.hash.to_string (), pending_tree);
						}
						else
...
void cga::rpc_handler::pending_exists ()
{
	auto hash (hash_impl ());
	const bool include_active = request.get<bool> ("include_active", false);
	if (!ec)
...
void cga::rpc_handler::payment_begin ()
{
	...
	if (!id.decode_hex (id_text))
	{
		...
		{
			auto transaction (node.wallets.tx_begin_write ());
			std::shared_ptr<cga::wallet> wallet (existing->second);
			if (wallet->store.valid_password (transaction))
			{
				...
				{
					ec = cga::error_rpc::payment_unable_create_account;
				}
			}
			else
			{
...
void cga::rpc_handler::payment_init ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::payment_end ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			{
				ec = cga::error_rpc::payment_account_balance;
			}
		}
	}
	response_errors ();
...
void cga::rpc_handler::payment_wait ()
{
	...
	if (!ec)
	{
		...
		{
			{
				auto observer (std::make_shared<cga::payment_observer> (response, rpc, account, amount));
		...
		if (!subtype_text.empty ())
		{
			...
			else
			{
				...
				if (subtype_text == "send")
				{
					...
					{
						ec = cga::error_rpc::invalid_subtype_balance;
					}
				...
				else if (subtype_text == "open")
				{
					...
					{
						ec = cga::error_rpc::invalid_subtype_previous;
					}
				}
				...
				else if (subtype_text == "epoch")
				{
					...
					{
						ec = ec = cga::error_rpc::invalid_subtype_epoch_link;
					}
				}
				else
				{
					ec = cga::error_rpc::invalid_subtype;
				}
			}
		}
	}
	if (!ec)
	{
		if (!cga::work_validate (*block))
		{
			...
			cga::process_return result;
			{
				...
				cga::unchecked_info info (block, block->account (), cga::seconds_since_epoch (), cga::signature_verification::unknown);
				result = node.block_processor.process_one (transaction, info);
			}
			switch (result.code)
			{
				case cga::process_result::progress:
				{
					response_l.put ("hash", hash.to_string ());
					break;
				}
				case cga::process_result::gap_previous:
				{
					ec = cga::error_process::gap_previous;
					break;
				}
				case cga::process_result::gap_source:
				{
					ec = cga::error_process::gap_source;
					break;
				}
				case cga::process_result::old:
				{
					ec = cga::error_process::old;
					break;
				}
				case cga::process_result::bad_signature:
				{
					ec = cga::error_process::bad_signature;
					break;
				}
				case cga::process_result::negative_spend:
				{
					// TODO once we get RPC versioning, this should be changed to "negative spend"
					ec = cga::error_process::negative_spend;
					break;
				}
				case cga::process_result::balance_mismatch:
				{
					ec = cga::error_process::balance_mismatch;
					break;
				}
				case cga::process_result::unreceivable:
				{
					ec = cga::error_process::unreceivable;
					break;
				}
				case cga::process_result::block_position:
				{
					ec = cga::error_process::block_position;
					break;
				}
				case cga::process_result::fork:
				{
					const bool force = request.get<bool> ("force", false);
					...
					{
						ec = cga::error_process::fork;
					}
					break;
				}
				default:
				{
					ec = cga::error_process::other;
					break;
				}
			}
		}
		...
		{
			ec = cga::error_blocks::work_low;
		}
	}
	response_errors ();
}

void cga::rpc_handler::receive ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	...
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			if (block != nullptr)
			{
				if (node.store.pending_exists (block_transaction, cga::pending_key (account, hash)))
				{
					auto work (work_optional_impl ());
					if (!ec && work)
					{
						cga::account_info info;
						cga::uint256_union head;
						if (!node.store.account_get (block_transaction, account, info))
						{
							head = info.head;
						}
						...
						if (cga::work_validate (head, work))
						{
							ec = cga::error_common::invalid_work;
						}
					}
					if (!ec)
					{
						...
						auto response_a (response);
						// clang-format off
						wallet->receive_async (std::move (block), account, cga::genesis_amount, [response_a](std::shared_ptr<cga::block> block_a) {
							cga::uint256_union hash_a (0);
							if (block_a != nullptr)
							{
								hash_a = block_a->hash ();
							}
...
void cga::rpc_handler::receive_minimum ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void cga::rpc_handler::receive_minimum_set ()
{
	rpc_control_impl ();
	auto amount (amount_impl ());
	if (!ec)
...
void cga::rpc_handler::representatives ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		{
			std::vector<std::pair<cga::uint128_union, std::string>> representation;
...
void cga::rpc_handler::representatives_online ()
{
	const auto accounts_node = request.get_child_optional ("accounts");
	const bool weight = request.get<bool> ("weight", false);
	std::vector<cga::public_key> accounts_to_filter;
...
void cga::rpc_handler::republish ()
{
	...
	if (!ec && sources_text.is_initialized ())
	{
		...
		{
			ec = cga::error_rpc::invalid_sources;
		}
	}
	...
	if (!ec && destinations_text.is_initialized ())
	{
		...
		{
			ec = cga::error_rpc::invalid_destinations;
		}
	}
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			std::deque<std::shared_ptr<cga::block>> republish_bundle;
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				block = node.store.block_get (transaction, hash);
				if (sources != 0) // Republish source chain
				{
					cga::block_hash source (node.ledger.block_source (transaction, *block));
					auto block_a (node.store.block_get (transaction, source));
					std::vector<cga::block_hash> hashes;
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					if (!destination.is_zero ())
					{
						if (!node.store.pending_exists (transaction, cga::pending_key (destination, hash)))
						{
							cga::block_hash previous (node.ledger.latest (transaction, destination));
...
void cga::rpc_handler::search_pending ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void cga::rpc_handler::search_pending_all ()
{
	rpc_control_impl ();
	if (!ec)
	{
		...
		if (!source.decode_account (source_text))
		{
			...
			if (!destination.decode_account (destination_text))
			{
				...
				if (!ec)
				{
					...
					auto response_a (response);
					// clang-format off
					wallet->send_async (source, destination, amount.number (), [balance, amount, response_a](std::shared_ptr<cga::block> block_a) {
						if (block_a != nullptr)
						{
							cga::uint256_union hash (block_a->hash ());
							boost::property_tree::ptree response_l;
							response_l.put ("block", hash.to_string ());
							response_a (response_l);
						}
						else
						{
							...
							{
								std::error_code ec (cga::error_common::insufficient_balance);
...
void cga::rpc_handler::sign ()
{
	...
	{
		ec = cga::error_rpc::sign_hash_disabled;
	}
	if (!ec)
	{
		cga::raw_key prv;
		...
		if (prv.data != 0)
		{
			...
			cga::signature signature (cga::sign_message (prv, pub, hash));
...
void cga::rpc_handler::stats ()
{
	...
	{
		ec = cga::error_rpc::invalid_missing_type;
	}
	if (!ec && use_sink)
	{
...
void cga::rpc_handler::stats_clear ()
{
	node.stats.clear ();
	response_l.put ("success", "");
	response (response_l);
...
void cga::rpc_handler::unchecked ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::unchecked_clear ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void cga::rpc_handler::unchecked_get ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::unchecked_keys ()
{
	auto count (count_optional_impl ());
	...
	if (!ec)
	{
		boost::property_tree::ptree unchecked;
		auto transaction (node.store.tx_begin_read ());
		for (auto i (node.store.unchecked_begin (transaction, cga::unchecked_key (key, 0))), n (node.store.unchecked_end ()); i != n && unchecked.size () < count; ++i)
		{
			boost::property_tree::ptree entry;
			cga::unchecked_info info (i->second);
			std::string contents;
			info.block->serialize_json (contents);
			entry.put ("key", cga::block_hash (i->first.key ()).to_string ());
...
void cga::rpc_handler::uptime ()
{
	response_l.put ("seconds", std::chrono::duration_cast<std::chrono::seconds> (std::chrono::steady_clock::now () - node.startup_time).count ());
	response_errors ();
}

void cga::rpc_handler::version ()
{
	response_l.put ("rpc_version", "1");
	response_l.put ("store_version", std::to_string (node.store_version ()));
	response_l.put ("protocol_version", std::to_string (cga::protocol_version));
	if (CGA_VERSION_PATCH == 0)
	{
		response_l.put ("node_vendor", boost::str (boost::format ("CGA %1%") % CGA_MAJOR_MINOR_VERSION));
	}
	else
	{
		response_l.put ("node_vendor", boost::str (boost::format ("CGA %1%") % CGA_MAJOR_MINOR_RC_VERSION));
	}
	response_errors ();
}

void cga::rpc_handler::validate_account_number ()
{
...
void cga::rpc_handler::wallet_add ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void cga::rpc_handler::wallet_add_watch ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void cga::rpc_handler::wallet_info ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		cga::uint128_t balance (0);
		cga::uint128_t pending (0);
		uint64_t count (0);
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			cga::key_type key_type (wallet->store.key_type (i->second));
			if (key_type == cga::key_type::deterministic)
			{
				deterministic_count++;
			}
			else if (key_type == cga::key_type::adhoc)
			{
				adhoc_count++;
			}
			count++;
...
void cga::rpc_handler::wallet_balances ()
{
	auto wallet (wallet_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		{
			cga::account account (i->first);
			cga::uint128_t balance = node.ledger.account_balance (block_transaction, account);
			if (balance >= threshold.number ())
			{
				boost::property_tree::ptree entry;
				cga::uint128_t pending = node.ledger.account_pending (block_transaction, account);
				entry.put ("balance", balance.convert_to<std::string> ());
...
void cga::rpc_handler::wallet_change_seed ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void cga::rpc_handler::wallet_contains ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void cga::rpc_handler::wallet_create ()
{
	rpc_control_impl ();
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			{
				ec = cga::error_common::wallet_lmdb_max_dbs;
			}
			if (!ec && seed_text.is_initialized ())
			{
				auto transaction (node.wallets.tx_begin_write ());
				cga::public_key account (wallet->change_seed (transaction, seed));
				response_l.put ("account", account.to_account ());
			}
		}
	}
...
void cga::rpc_handler::wallet_destroy ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void cga::rpc_handler::wallet_export ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::wallet_frontiers ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::wallet_history ()
{
...
void cga::rpc_handler::wallet_key_valid ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::wallet_ledger ()
{
	const bool representative = request.get<bool> ("representative", false);
	const bool weight = request.get<bool> ("weight", false);
	const bool pending = request.get<bool> ("pending", false);
...
void cga::rpc_handler::wallet_lock ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
...
void cga::rpc_handler::wallet_pending ()
{
	auto wallet (wallet_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			cga::account account (i->first);
			boost::property_tree::ptree peers_l;
			for (auto ii (node.store.pending_begin (block_transaction, cga::pending_key (account, 0))); cga::pending_key (ii->first).account == account && peers_l.size () < count; ++ii)
			{
				cga::pending_key key (ii->first);
				std::shared_ptr<cga::block> block (include_active ? nullptr : node.store.block_get (block_transaction, key.hash));
				if (include_active || (block && !node.active.active (*block)))
				{
					if (threshold.is_zero () && !source)
					{
					...
					{
						cga::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source || min_version)
							{
...
void cga::rpc_handler::wallet_representative ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void cga::rpc_handler::wallet_representative_set ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		if (!representative.decode_account (representative_text))
		{
			...
			if (!ec && update_existing_accounts)
			{
				...
				{
					// clang-format off
					wallet->change_async (account, representative, [](std::shared_ptr<cga::block>) {}, 0, false);
					// clang-format on
				}
			}
		}
...
void cga::rpc_handler::wallet_republish ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto count (count_impl ());
...
void cga::rpc_handler::wallet_work_get ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			(void)error_work;
			works.put (account.to_account (), cga::to_string_hex (work));
		}
...
void cga::rpc_handler::work_generate ()
{
	rpc_control_impl ();
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		auto callback = [rpc_l](boost::optional<uint64_t> const & work_a) {
			...
			{
				boost::property_tree::ptree response_l;
				response_l.put ("work", cga::to_string_hex (work_a.value ()));
...
void cga::rpc_handler::work_cancel ()
{
	rpc_control_impl ();
	auto hash (hash_impl ());
	if (!ec)
...
void cga::rpc_handler::work_get ()
{
	...
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			auto error_work (wallet->store.work_get (transaction, account, work));
			(void)error_work;
			response_l.put ("work", cga::to_string_hex (work));
		}
	}
	response_errors ();
}

void cga::rpc_handler::work_set ()
{
	rpc_control_impl ();
	auto wallet (wallet_impl ());
	auto account (account_impl ());
...
void cga::rpc_handler::work_validate ()
{
	auto hash (hash_impl ());
	auto work (work_optional_impl ());
	if (!ec)
	{
		auto validate (cga::work_validate (hash, work));
		response_l.put ("valid", validate ? "0" : "1");
	}
	response_errors ();
}

void cga::rpc_handler::work_peer_add ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void cga::rpc_handler::work_peers ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
void cga::rpc_handler::work_peers_clear ()
{
	rpc_control_impl ();
	if (!ec)
	{
...
cga::rpc_connection::rpc_connection (cga::node & node_a, cga::rpc & rpc_a) :
node (node_a.shared ()),
rpc (rpc_a),
...
void cga::rpc_connection::parse_connection ()
{
	read ();
}

void cga::rpc_connection::prepare_head (unsigned version, boost::beast::http::status status)
{
	res.version (version);
	res.result (status);
...
void cga::rpc_connection::write_result (std::string body, unsigned version, boost::beast::http::status status)
{
		...
		if (!ec)
		{
			this_l->node->background ([this_l]() {
				...
				switch (method)
				{
					...
					{
						auto handler (std::make_shared<cga::rpc_handler> (*this_l->node, this_l->rpc, this_l->request.body (), request_id, response_handler));
...
void cga::rpc_handler::process_request ()
{
	try
	{
		...
		else
		{
			...
			else if (action == "kcga_from_raw" || action == "krai_from_raw")
			{
				mcga_from_raw (cga::kcga_ratio);
			}
			else if (action == "kcga_to_raw" || action == "krai_to_raw")
			{
				mcga_to_raw (cga::kcga_ratio);
			}
			else if (action == "ledger")
			{
				ledger ();
			}
			else if (action == "mcga_from_raw" || action == "mrai_from_raw")
			{
				mcga_from_raw ();
			}
			else if (action == "mcga_to_raw" || action == "mrai_to_raw")
			{
				mcga_to_raw ();
			}
			...
			else if (action == "cga_from_raw" || action == "rai_from_raw")
			{
				mcga_from_raw (cga::cga_ratio);
			}
			else if (action == "cga_to_raw" || action == "rai_to_raw")
			{
				mcga_to_raw (cga::cga_ratio);
			}
...
cga::payment_observer::payment_observer (std::function<void(boost::property_tree::ptree const &)> const & response_a, cga::rpc & rpc_a, cga::account const & account_a, cga::amount const & amount_a) :
rpc (rpc_a),
account (account_a),
amount (amount_a),
response (response_a)
...
void cga::payment_observer::start (uint64_t timeout)
{
	auto this_l (shared_from_this ());
	rpc.node.alarm.add (std::chrono::steady_clock::now () + std::chrono::milliseconds (timeout), [this_l]() {
		this_l->complete (cga::payment_status::nothing);
	});
}

cga::payment_observer::~payment_observer ()
{
}

void cga::payment_observer::observe ()
{
	if (rpc.node.balance (account) >= amount.number ())
	{
		complete (cga::payment_status::success);
	}
}

void cga::payment_observer::complete (cga::payment_status status)
{
	...
	if (!already)
	{
		...
		switch (status)
		{
			case cga::payment_status::nothing:
			{
				boost::property_tree::ptree response_l;
			...
			case cga::payment_status::success:
			{
				boost::property_tree::ptree response_l;
...
std::unique_ptr<cga::rpc> cga::get_rpc (boost::asio::io_context & io_ctx_a, cga::node & node_a, cga::rpc_config const & config_a)
{
...
void construct_json (cga::seq_con_info_component * component, boost::property_tree::ptree & parent)
{
	...
	{
		auto & leaf_info = static_cast<cga::seq_con_info_leaf *> (component)->get_info ();
	...
	auto composite = static_cast<cga::seq_con_info_composite *> (component);
