...
public:
	...
	QPushButton * back;
	xpd_qt::wallet & wallet;
};
class advanced_actions
{
public:
	advanced_actions (xpd_qt::wallet &);
	QWidget * window;
	QVBoxLayout * layout;
	...
	QRadioButton * mxpd_unit;
	QRadioButton * kxpd_unit;
	QRadioButton * xpd_unit;
	QPushButton * back;
...
{
public:
	self_pane (xpd_qt::wallet &, xpeed::account const &);
	void set_balance_text (std::pair<xpeed::uint128_t, xpeed::uint128_t>);
	QWidget * window;
	QVBoxLayout * layout;
...
{
public:
	history (xpeed::ledger &, xpeed::account const &, xpd_qt::wallet &);
	void refresh ();
	QWidget * window;
	QVBoxLayout * layout;
	QStandardItemModel * model;
	...
	QSpinBox * tx_count;
	xpeed::ledger & ledger;
	xpeed::account const & account;
	xpd_qt::wallet & wallet;
};
class block_viewer
{
public:
	block_viewer (xpd_qt::wallet &);
	void rebroadcast_action (xpeed::uint256_union const &);
	QWidget * window;
	QVBoxLayout * layout;
	QLabel * hash_label;
	QLineEdit * hash;
...
{
public:
	status (xpd_qt::wallet &);
	void erase (xpd_qt::status_types);
	void insert (xpd_qt::status_types);
	void set_text ();
	std::string text ();
	std::string color ();
	std::set<xpd_qt::status_types> active;
	xpd_qt::wallet & wallet;
};
class wallet : public std::enable_shared_from_this<xpd_qt::wallet>
{
public:
	wallet (QApplication &, xpd_qt::eventloop_processor &, xpeed::node &, std::shared_ptr<xpeed::wallet>, xpeed::account &);
	void start ();
	void refresh ();
	void update_connected ();
	void empty_password ();
	void change_rendering_ratio (xpeed::uint128_t const &);
	std::string format_balance (xpeed::uint128_t const &) const;
	xpeed::uint128_t rendering_ratio;
	xpeed::node & node;
	std::shared_ptr<xpeed::wallet> wallet_m;
	xpeed::account & account;
	xpd_qt::eventloop_processor & processor;
	xpd_qt::history history;
	xpd_qt::accounts accounts;
	xpd_qt::self_pane self;
	xpd_qt::settings settings;
	xpd_qt::advanced_actions advanced;
	xpd_qt::block_creation block_creation;
	xpd_qt::block_entry block_entry;
	xpd_qt::block_viewer block_viewer;
	xpd_qt::account_viewer account_viewer;
	xpd_qt::stats_viewer stats_viewer;
	xpd_qt::import import;
