...
bool xpd_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<xpd_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<xpd_qt::eventloop_event *> (event_a)->action ();
	return true;
}

xpd_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

xpd_qt::self_pane::self_pane (xpd_qt::wallet & wallet_a, xpeed::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel ("Your Xpeed account:")),
...
wallet (wallet_a)
{
	...
	if (xpeed::is_beta_network)
	{
		network = "Beta";
	}
	else if (xpeed::is_test_network)
	{
		network = "Test";
	}
	if (XPD_VERSION_PATCH == 0)
	{
		version = new QLabel (boost::str (boost::format ("Version %1% %2% network") % XPD_MAJOR_MINOR_VERSION % network).c_str ());
	}
	else
	{
		version = new QLabel (boost::str (boost::format ("Version %1% %2% network") % XPD_MAJOR_MINOR_RC_VERSION % network).c_str ());
	}
...
void xpd_qt::self_pane::set_balance_text (std::pair<xpeed::uint128_t, xpeed::uint128_t> balance_a)
{
...
xpd_qt::accounts::accounts (xpd_qt::wallet & wallet_a) :
...
void xpd_qt::accounts::refresh_wallet_balance ()
{
	auto transaction (this->wallet.wallet_m->wallets.tx_begin_read ());
	auto block_transaction (this->wallet.node.store.tx_begin_read ());
	xpeed::uint128_t balance (0);
	xpeed::uint128_t pending (0);
...
xpd_qt::import::import (xpd_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
xpd_qt::history::history (xpeed::ledger & ledger_a, xpeed::account const & account_a, xpd_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
namespace
{
class short_text_visitor : public xpeed::block_visitor
{
public:
	short_text_visitor (xpeed::transaction const & transaction_a, xpeed::ledger & ledger_a) :
	transaction (transaction_a),
	ledger (ledger_a)
	{
	}
	void send_block (xpeed::send_block const & block_a)
	{
		type = "Send";
		account = block_a.hashables.destination;
		amount = ledger.amount (transaction, block_a.hash ());
	}
	void receive_block (xpeed::receive_block const & block_a)
	{
		type = "Receive";
		account = ledger.account (transaction, block_a.source ());
		amount = ledger.amount (transaction, block_a.source ());
	}
	void open_block (xpeed::open_block const & block_a)
	{
		type = "Receive";
		...
		{
			account = xpeed::genesis_account;
			amount = xpeed::genesis_amount;
		}
	}
	void change_block (xpeed::change_block const & block_a)
	{
		type = "Change";
		amount = 0;
		account = block_a.hashables.representative;
	}
	void state_block (xpeed::state_block const & block_a)
	{
...
xpd_qt::block_viewer::block_viewer (xpd_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void xpd_qt::block_viewer::rebroadcast_action (xpeed::uint256_union const & hash_a)
{
	auto done (true);
...
xpd_qt::account_viewer::account_viewer (xpd_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (XPD): ") + wallet.format_balance (balance.first));
...
xpd_qt::stats_viewer::stats_viewer (xpd_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
refresh (new QPushButton ("Refresh")),
model (new QStandardItemModel),
...
void xpd_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
xpd_qt::status::status (xpd_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (xpd_qt::status_types::nominal);
	set_text ();
}

void xpd_qt::status::erase (xpd_qt::status_types status_a)
{
	assert (status_a != xpd_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void xpd_qt::status::insert (xpd_qt::status_types status_a)
{
	assert (status_a != xpd_qt::status_types::nominal);
...
std::string xpd_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case xpd_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case xpd_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case xpd_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case xpd_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case xpd_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case xpd_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case xpd_qt::status_types::nominal:
...
std::string xpd_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
xpd_qt::wallet::wallet (QApplication & application_a, xpd_qt::eventloop_processor & processor_a, xpeed::node & node_a, std::shared_ptr<xpeed::wallet> wallet_a, xpeed::account & account_a) :
rendering_ratio (xpeed::Mxpd_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
needs_deterministic_restore (false)
{
	...
	send_account->setPlaceholderText (xpeed::zero_key.pub.to_account ().c_str ());
...
void xpd_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<xpd_qt::wallet> wallet_w (shared_from_this ());
...
void xpd_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<xpd_qt::wallet> this_w (shared_from_this ());
	...
	QObject::connect (send_blocks_send, &QPushButton::released, [this_w]() {
		if (auto this_l = this_w.lock ())
		{
			...
			{
				xpeed::uint128_t actual (amount.number () * this_l->rendering_ratio);
						...
						if (actual <= balance)
						{
							...
							if (this_l->wallet_m->store.valid_password (transaction))
							{
								...
								this_l->node.background ([this_w, account_l, actual]() {
									if (auto this_l = this_w.lock ())
									{
										this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<xpeed::block> block_a) {
											if (auto this_l = this_w.lock ())
											{
	...
	node.observers.blocks.add ([this_w](std::shared_ptr<xpeed::block> block_a, xpeed::account const & account_a, xpeed::uint128_t const & amount_a, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w](xpeed::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (xpd_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (xpd_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](xpeed::endpoint const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (xpd_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (xpd_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (xpd_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (xpd_qt::status_types::working);
					}
				}
			}));
		}
...
void xpd_qt::wallet::update_connected ()
{
	if (node.peers.empty ())
	{
		active_status.insert (xpd_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (xpd_qt::status_types::disconnected);
	}
}

void xpd_qt::wallet::empty_password ()
{
...
void xpd_qt::wallet::change_rendering_ratio (xpeed::uint128_t const & rendering_ratio_a)
{
...
std::string xpd_qt::wallet::format_balance (xpeed::uint128_t const & balance) const
{
	auto balance_str = xpeed::amount (balance).format_balance (rendering_ratio, 0, false);
	auto unit = std::string ("XPD");
	if (rendering_ratio == xpeed::kxpd_ratio)
	{
		unit = std::string ("kxpd");
	}
	else if (rendering_ratio == xpeed::xpd_ratio)
	{
		unit = std::string ("xpd");
	}
...
void xpd_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void xpd_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

xpd_qt::settings::settings (xpd_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
wallet (wallet_a)
{
	...
	new_representative->setPlaceholderText (xpeed::zero_key.pub.to_account ().c_str ());
	...
	QObject::connect (change_rep, &QPushButton::released, [this]() {
		xpeed::account representative_l;
...
void xpd_qt::settings::refresh_representative ()
{
...
void xpd_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void xpd_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (xpd_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (xpd_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (xpd_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (xpd_qt::status_types::vulnerable);
	}
}

xpd_qt::advanced_actions::advanced_actions (xpd_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
mxpd_unit (new QRadioButton ("Mxpd")),
kxpd_unit (new QRadioButton ("kxpd")),
xpd_unit (new QRadioButton ("xpd")),
back (new QPushButton ("Back")),
...
bootstrap_label (new QLabel ("IPV6:port \"::ffff:192.168.0.1:7077\"")),
...
wallet (wallet_a)
{
	ratio_group->addButton (mxpd_unit);
	ratio_group->addButton (kxpd_unit);
	ratio_group->addButton (xpd_unit);
	ratio_group->setId (mxpd_unit, 0);
	ratio_group->setId (kxpd_unit, 1);
	ratio_group->setId (xpd_unit, 2);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mxpd_unit);
	scale_layout->addWidget (kxpd_unit);
	scale_layout->addWidget (xpd_unit);
	...
	QObject::connect (mxpd_unit, &QRadioButton::toggled, [this]() {
		if (mxpd_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (mxpd_unit));
			this->wallet.change_rendering_ratio (xpeed::Mxpd_ratio);
		}
	});
	QObject::connect (kxpd_unit, &QRadioButton::toggled, [this]() {
		if (kxpd_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (kxpd_unit));
			this->wallet.change_rendering_ratio (xpeed::kxpd_ratio);
		}
	});
	QObject::connect (xpd_unit, &QRadioButton::toggled, [this]() {
		if (xpd_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (xpd_unit));
			this->wallet.change_rendering_ratio (xpeed::xpd_ratio);
		}
	});
	auto selected_ratio_id (QSettings ().value (saved_ratio_key, ratio_group->id (mxpd_unit)).toInt ());
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this]() {
		...
		auto error (xpeed::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void xpd_qt::advanced_actions::refresh_peers ()
{
...
void xpd_qt::advanced_actions::refresh_ledger ()
{
	...
	{
		QList<QStandardItem *> items;
		items.push_back (new QStandardItem (QString (xpeed::block_hash (i->first).to_account ().c_str ())));
		xpeed::account_info info (i->second);
		std::string balance;
		xpeed::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void xpd_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

xpd_qt::block_entry::block_entry (xpd_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this]() {
		...
		try
		{
			...
			auto block_l (xpeed::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
xpd_qt::block_creation::block_creation (xpd_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void xpd_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void xpd_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void xpd_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void xpd_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void xpd_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						xpeed::state_block send (account_l, info.head, rep_block->representative (), balance - amount_l.number (), destination_l, key, account_l, 0);
...
void xpd_qt::block_creation::create_receive ()
{
	xpeed::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				xpeed::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							auto rep_block (wallet.node.store.block_get (block_transaction, info.rep_block));
							assert (rep_block != nullptr);
							xpeed::state_block receive (pending_key.account, info.head, rep_block->representative (), info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
...
void xpd_qt::block_creation::create_change ()
{
	xpeed::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					xpeed::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
...
void xpd_qt::block_creation::create_open ()
{
	xpeed::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								xpeed::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
