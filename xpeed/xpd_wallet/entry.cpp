...
public:
	...
	bool upgrade_json (unsigned version_a, xpeed::jsonconfig & json)
	{
	...
	xpeed::error deserialize_json (bool & upgraded_a, xpeed::jsonconfig & json)
	{
		if (!json.empty ())
		{
	...
	void serialize_json (xpeed::jsonconfig & json)
	{
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path, xpeed::node_flags const & flags)
{
	...
	if (!error)
	{
		...
		std::shared_ptr<xpd_qt::wallet> gui;
		xpeed::set_application_icon (application);
		auto opencl (xpeed::opencl_work::create (config.opencl_enable, config.opencl, config.node.logging));
		xpeed::work_pool work (config.node.work_threads, opencl ? [&opencl](xpeed::uint256_union const & root_a) {
			return opencl->generate_work (root_a);
		}
		                                                       : std::function<boost::optional<uint64_t> (xpeed::uint256_union const &)> (nullptr));
		xpeed::alarm alarm (io_ctx);
		xpeed::node_init init;
		node = std::make_shared<xpeed::node> (init, io_ctx, data_path, alarm, config.node, work, flags);
		if (!init.error ())
		{
			...
			if (config.account.is_zero () || !wallet->exists (config.account))
			{
				...
				{
					xpeed::uint256_union account (existing->first);
			...
			xpeed::thread_runner runner (io_ctx, node->config.io_threads);
			QObject::connect (&application, &QApplication::aboutToQuit, [&]() {
				ipc.stop ();
				rpc->stop ();
				node->stop ();
			});
			application.postEvent (&processor, new xpd_qt::eventloop_event ([&]() {
				gui = std::make_shared<xpd_qt::wallet> (application, processor, *node, wallet, config.account);
				splash->close ();
				gui->start ();
...
int main (int argc, char * const * argv)
{
	...
	try
	{
		...
		xpeed::add_node_options (description);
		...
		if (!vm.count ("data_path"))
		{
			...
			if (!xpeed::migrate_working_path (error_string))
			{
				throw std::runtime_error (error_string);
			}
		}

		auto ec = xpeed::handle_node_options (vm);
		if (ec == xpeed::error_cli::unknown_command)
		{
			if (vm.count ("help") != 0)
			{
